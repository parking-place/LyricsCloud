...
test.describe("1.0.1 P7 brand and build metadata", () => {
  test("uses the approved theme logo and the same runtime metadata as health", async ({ page }, testInfo) => {
    ...
    await expect(page.locator("#runtime-build-label")).toHaveText("v1.0.1-p9 dev");
    ...
    expect(`v${live.build.version}-${live.build.phase} ${live.build.channel}`).toBe("v1.0.1-p9 dev");
