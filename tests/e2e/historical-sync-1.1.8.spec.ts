...
test("ES-03 keeps a remote title insertion while local IME replaces a different range", async ({ context, page }) => {
  test.skip(!process.env.E2E_DATABASE_URL, "requires isolated E2E database");
  const userId = randomUUID(); const token = `historical-title-${randomUUID()}`;
  await withE2eDatabase(async (pool) => {
    await pool.query("insert into app_users(id,status) values($1,'active')", [userId]);
    await pool.query("insert into user_profiles(owner_id,display_name) values($1,'제목 동기화 사용자')", [userId]);
    await pool.query("insert into auth_sessions(token_hash,user_id,expires_at,absolute_expires_at) values($1,$2,now()+interval '1 hour',now()+interval '2 hours')", [hashToken(token), userId]);
  });
  await context.addCookies([{ name: "lc_session", value: token, url: origin, httpOnly: true, sameSite: "Lax" }]);
  const second = await context.newPage();
  try {
    const created = await page.request.post("/api/prompts", { headers: { Origin: origin }, data: {
      requestId: randomUUID(), title: "hello world", mode: "sentence", sentenceText: "ambient" } });
    expect(created.status()).toBe(201);
    const id = (await created.json()).prompt.id as string;
    await Promise.all([page.goto(`/prompts/${id}`), second.goto(`/prompts/${id}`)]);
    for (const tab of [page, second]) await expect(tab.getByText("방금 저장됨", { exact: true })).toBeVisible();
    ...
    await title.dispatchEvent("compositionend", { data: "WORLD" });
    await expect(title).toHaveValue("hello dear WORLD");
    await expect(second.getByRole("textbox", { name: "프롬프트 제목" })).toHaveValue("hello dear WORLD");
    await expect.poll(async () => (await (await page.request.get(`/api/prompts/${id}`)).json()).prompt.title).toBe("hello dear WORLD");
  } finally {
    await second.close();
    await withE2eDatabase(async (pool) => { await pool.query("delete from app_users where id=$1", [userId]); });
