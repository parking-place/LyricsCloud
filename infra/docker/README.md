...
- GitHub Actions는 전체 검증 뒤 네 target을 `parkingplace/lyricscloud:<version>-beta.<commit>-<service>`, 버전별 이동식 beta tag, `beta-latest-<service>` alias로 발행한다. 기본 `beta-latest`는 web image를 가리킨다.
