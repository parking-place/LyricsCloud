...
for (const marker of ["NF-REQ-035", "NF-REQ-036", `AC-${currentVersion}-01`, `AC-${currentVersion}-04`, "지정 사용자", "공동 편집", "P0/P1", "OPS-100-001", "1120_selected_lyric_write.sql"]) {
...
for (const marker of ["지정 사용자", "공동 편집", "복구", "실제 OS", "알려진 제한", "OPS-100-001"]) {
...
assert(formalManifest.releaseVersion === currentVersion && formalManifest.releaseChannel === "release"
  && formalManifest.productionAuthorized === true && formalManifest.database.requiredLatestSchema === "1120_selected_lyric_write.sql",
