...
const formalTraceability = read("docs/architecture/1.0.3-FINAL-TRACEABILITY.md");
const formalReleaseNotes = read("docs/releases/1.0.3.md");
const formalReleaseRunbook = read("docs/runbooks/1.0.3-release.md");
const formalReleaseChecklist = read("docs/runbooks/1.0.3-release-checklist.md");
const formalManifest = JSON.parse(read("config/release-manifest.1.0.3.json"));
...
for (const marker of ["1.0.4 Phase 5", "1.0.3 release notes", "CHANGELOG.md"]) {
...
const currentDevTags = getImagePublication({ eventName: "push", refType: "branch", refName: "phase/1.0.4-p5-prompt-copy-release-handoff",
...
for (const marker of ["APP_VERSION: 1.0.4", "APP_PHASE: p5", "test:migration:1000", "test:environment:101"]) {
  assert(workflow.includes(marker), `P5 CI marker missing: ${marker}`);
}
for (const marker of ["P5", "P0/P1", "OPS-100-001", "동일 SHA 개발 공개 smoke"]) {
  assert(formalTraceability.includes(marker), `1.0.3 traceability marker missing: ${marker}`);
}
for (const marker of ["프롬프트 표현 모드", "원문", "capability", "알려진 제한", "OPS-100-001"]) {
  assert(formalReleaseNotes.includes(marker), `1.0.3 release notes marker missing: ${marker}`);
}
for (const marker of ["annotated `v1.0.3`", "publish=true", "release=true", "Release-latest", "application-first rollback"]) {
  assert(formalReleaseRunbook.includes(marker), `1.0.3 release runbook marker missing: ${marker}`);
}
for (const marker of ["main", "annotated `v1.0.3`", "exact digest", "OPS-100-001"]) {
  assert(formalReleaseChecklist.includes(marker), `1.0.3 release checklist marker missing: ${marker}`);
}
assert(formalManifest.releaseVersion === "1.0.3" && formalManifest.releaseChannel === "release"
  && formalManifest.productionAuthorized === true && formalManifest.database.requiredLatestSchema === "1000_prompt_modes.sql",
  "1.0.3 formal release manifest boundary invalid");
const formalReleaseTags = getImagePublication({ eventName: "workflow_dispatch", refType: "tag", refName: "v1.0.3",
  sha: "c".repeat(40), version: "1.0.3", release: true }).tags;
for (const tag of ["1.0.3", "Release", "latest", "Release-latest"]) {
  assert(formalReleaseTags.includes(tag), `1.0.3 release tag missing: ${tag}`);
}
