...
for (const marker of ["NF-REQ-034", "NF-REQ-036", `AC-${currentVersion}-01`, `AC-${currentVersion}-04`, "selected-read", "P0/P1", "OPS-100-001", "1100_selected_lyric_sharing.sql"]) {
...
for (const marker of ["지정 사용자", "읽기 전용", "권한 회수", "실제 OS", "알려진 제한", "OPS-100-001"]) {
...
assert(formalManifest.releaseVersion === currentVersion && formalManifest.releaseChannel === "release"
  && formalManifest.productionAuthorized === true && formalManifest.database.requiredLatestSchema === "1100_selected_lyric_sharing.sql",
