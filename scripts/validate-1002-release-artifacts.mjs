...
const formalVersion = "1.0.2";
assert(version === "1.0.3", "VERSION must be the active 1.0.3 patch candidate");
...
assertIncludes(status, 'current_version: "1.0.3"', "STATUS current version does not match VERSION");
const runtime = await read("packages/config/src/index.ts");
assertIncludes(runtime, 'appVersion: env.APP_VERSION ?? "1.0.3"', "runtime default version is not current");
assertIncludes(await read("apps/web/next.config.ts"), 'generateBuildId: async () => process.env.NEXT_BUILD_ID ?? "lyricscloud-1.0.3"', "deterministic Next build ID missing");
for (const path of ["compose.yaml", "compose.backup.yaml", ".env.example"]) assertIncludes(await read(path), "1.0.3", `${path} lacks 1.0.3`);
...
assert(formalMigrations.applyOrder.length <= migrationFiles.length && formalMigrations.latestSchema === "0901_beta_signup.sql",
  "1.0.2 sealed migration manifest is invalid");
...
assert(formalLicenses.productVersion === formalVersion && formalLicenses.lockfileSha256 === hash(lockfile), "1.0.2 formal license inventory invalid");
...
console.log(`1.0.3 runtime candidate plus sealed 1.0.2, 1.0.1 and 1.0.0 contracts: ${packages.length} package versions, ${migrationFiles.length} migrations, 4 digest-only signed images verified`);
