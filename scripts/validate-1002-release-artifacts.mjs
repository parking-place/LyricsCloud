...
const formalVersion = "1.0.7";
assert(version === "1.0.7", "VERSION must be the current 1.0.7 candidate source");
...
assertIncludes(status, 'current_version: "1.0.7"', "STATUS current version does not match VERSION");
const runtime = await read("packages/config/src/index.ts");
assertIncludes(runtime, 'appVersion: env.APP_VERSION ?? "1.0.7"', "runtime default version is not current");
assertIncludes(await read("apps/web/next.config.ts"), 'generateBuildId: async () => process.env.NEXT_BUILD_ID ?? "lyricscloud-1.0.7"', "deterministic Next build ID missing");
for (const path of ["compose.yaml", "compose.backup.yaml", ".env.example"]) assertIncludes(await read(path), "1.0.7", `${path} lacks 1.0.7`);
...
const formalEnvironmentText = await read("config/environment-schema.1.0.7.json");
const formalMigrationsText = await read("config/migrations.1.0.7.json");
const formalLicensesText = await read("config/licenses.1.0.7.json");
...
const formalManifest = await json("config/release-manifest.1.0.7.json");
assert(formalEnvironment.properties.APP_VERSION.const === formalVersion && formalEnvironment.properties.APP_CHANNEL.const === "release",
  "1.0.7 formal environment boundary invalid");
assert(formalMigrations.productVersion === formalVersion && formalMigrations.maximumCompatibleApplicationVersion === formalVersion,
  "1.0.7 formal migration compatibility invalid");
assert(formalMigrations.applyOrder.length <= migrationFiles.length && formalMigrations.latestSchema === "1001_library_view_settings.sql",
  "1.0.7 sealed migration manifest is invalid");
for (const entry of formalMigrations.applyOrder) {
  assert(hash(await read(`packages/database/migrations/${entry.name}`)) === entry.sha256, `${entry.name} 1.0.7 checksum changed`);
}
assert(formalLicenses.productVersion === formalVersion && formalLicenses.lockfileSha256 === hash(lockfile), "1.0.7 formal license inventory invalid");
assert(Object.values(formalLicenses.licenses).flat().length === formalLicenses.totalPackages,
  "1.0.7 formal license package count changed");
assert(formalLicenses.review.unknownLicenses.length === 0 && formalLicenses.review.blockedLicenses.length === 0,
  "1.0.7 formal license review has blockers");
...
assert(formalManifest.database.manifestSha256 === hash(formalMigrationsText)
  ...
  && formalManifest.licenses.inventorySha256 === hash(formalLicensesText), "1.0.7 formal artifact checksums differ");
for (const service of ["web", "collaboration", "worker", "migrate"]) {
  assert(formalManifest.images[service].repository === `parkingplace/lyricscloud-${service}`, `${service} 1.0.7 repository invalid`);
  assert(formalManifest.images[service].digest === `$${service.toUpperCase()}_DIGEST`, `${service} 1.0.7 digest placeholder invalid`);
}

console.log(`1.0.7 approved release plus sealed 1.0.5/1.0.3/1.0.1/1.0.0 contracts: ${packages.length} package versions, ${migrationFiles.length} migrations, 4 digest-only signed images verified`);
