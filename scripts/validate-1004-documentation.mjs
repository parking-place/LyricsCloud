...
for (const [document, markers] of [
  ...
  [changelog, ["## [1.0.3] - 2026-09-10", "## [1.0.2] - 2026-09-10", "Known limitations"]],
  [readme, ["apps/web/public/icons/lyricscloud-mark-dark.svg", "actions/workflows/ci.yml/badge.svg", "1.0.3 Phase 5", "LyricsCloud betacode", "원문 보존"]],
...
if (failures.length) {
  console.error(`1.0.3 current documentation validation failed (${failures.length})`);
  for (const failure of failures) console.error(`- ${failure}`);
  process.exit(1);
}

console.log(`1.0.3 current documentation validation PASS (${markdownFiles.length} Markdown files, ${uiNames.length} screens)`);
