...
cleanup() {
  docker rm -f "$name-web" "$name-collaboration" "$name-worker" "$name-migrate" "$name-db" >/dev/null 2>&1 || true
  docker network rm "$name" >/dev/null 2>&1 || true
}
trap cleanup EXIT
docker network create "$name" >/dev/null
docker run -d --name "$name-db" --network "$name" --network-alias db --tmpfs /var/lib/postgresql \
...
for service in collaboration worker web; do
  docker run -d --name "$name-$service" --network "$name" --network-alias "$service" \
    -e "DATABASE_URL=$database_url" "lyricscloud-$service-ci:$revision" >/dev/null
done
check_http() {
