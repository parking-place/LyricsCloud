...
for (const marker of ["COSIGN_EXPERIMENTAL=1", "cosign sign", "registry-referrers-mode=oci-1-1", "cosign verify", "experimental-oci11", "certificate-identity", "certificate-oidc-issuer", "Provenance.SLSA", "vcs:source", "configSource.path", "GITHUB_SHA"]) {
...
if (process.env.APP_CHANNEL === "release" || process.env.REQUIRE_CURRENT_RELEASE_SEAL === "true") {
  ...
  console.log(`Current ${currentVersion} release-document sealing requires an explicit release/final-seal intent.`);
}
