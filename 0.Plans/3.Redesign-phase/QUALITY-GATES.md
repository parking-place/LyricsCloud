...
비교 브랜치는 [고정 SHA 비교표](BRANCH-COMPARISON-118-P4.md)를 따른다. 그 브랜치의 코드/테스트/CI 기록은 **그 source SHA의 근거**다. 현재 b 또는 1.2.x로 포팅한 SHA에 자동 승계하지 않는다. `미변경 / 부분 대응 / 해결 후보 / 채택 후 검증 완료 / 미확인`을 구별한다. 이 계획 작성 시 `채택 후 검증 완료`인 항목은 없다.
...
## 1.1.7b 최초 통합·1.2.0 전환 차단 검수

| 항목 | 최초 우선순위·근거 | b 최초 통합 / 1.2.0 전환에서 필요한 판정 | 상세 후속 소유 |
|---|---|---|---|
...
후속 배정은 출시 예외가 아니다. 재현 환경이 없거나 후보 수정이 아직 미검증이면 해결로 닫지 않는다. 1.1.7b P1에서 [22원인 차단표](1.1.7.b/BLOCKERS.md)를 인수하고 필요한 최소 보정의 범위/담당을 정해 b P4/P5에서 증거로 닫는다. 위 표의 기존 1.2.x 소유는 후속 잔여 담당이며, b의 필수 차단 해소를 뒤로 미루는 근거가 아니다. 1.2.0은 인수된 b SHA의 동작을 디자인 전환에서 재검증한다. 기존 `OPS-100-001`은 별도의 역사적 위험 예외이며 잠금 수정만으로 해소되지 않는다. 새 릴리스의 적용 가능성은 [릴리스 정책](../2.Patch-phase/RELEASE-POLICY.md)대로 재평가하고 과거 예외를 자동 연장하지 않는다.
...
마지막 Phase는 전체 증거를 봉인한다. 앞 Phase의 CI/개발 인수를 마지막 Phase까지 모두 미뤄도 된다는 뜻이 아니다. 해당 Phase의 필수 gate가 열려 있거나 신규/악화 회귀가 있으면 `review/blocked`와 원인을 남기고 다음 Phase를 시작하지 않는다. b P1에 등록한 기존 차단은 담당 수정 Phase·노출 제한·인수 조건을 [차단표](1.1.7.b/BLOCKERS.md)에 명시해 추적하며 P4/P5에서 모두 닫는다. 중간 개발 인수를 정식 사용자 공개 승인으로 쓰지 않는다. 노출 통제가 불가능하면 앞 단계에서도 중단한다. 정확한 a/b 예외·3.Redesign-phase 경로·선언된 Phase 수와 dev/정식 검증 분리는 **1.1.7b P1**이 처음 지원한다. 1.2.0 P1은 도구/회귀를 인수해 실제 버전 전환을 수행하고, 1.2.2의 6 Phase는 이 기반의 회귀와 실제 P6 배선을 다시 확인한다.
