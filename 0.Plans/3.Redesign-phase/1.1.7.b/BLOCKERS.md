...
원격 안정화와 원래 0922 리뷰는 범위가 다르다. 정적 비교의 **미변경 14·부분 대응 5·해결 후보 3**은 당시 진입 판정이다. P4 기능 SHA `ddc1d7c50a64f8e8467b7aee64627f2dfe13da7d`에서 아래 22행을 다시 판정해 적용 개발 환경의 필수 P0/P1 차단은 닫았다. UI-04/05/07의 P2 잔여는 해결로 쓰지 않고, 실제 기기·OS IME·AT는 사용자 보류/미실행이다. 새 손실·거짓 저장·인가·복구 위반 근거가 나오면 판정을 다시 연다.
...
### 2026-09-24 P4 최종 판정 — 기능 SHA `ddc1d7c50a64f8e8467b7aee64627f2dfe13da7d`

아래 중간 표의 22행을 같은 기능 SHA의 전체 DB Vitest 509 PASS/8 조건부 skip, production Chromium 432 PASS/54 조건부 skip, 교차 엔진 20 PASS 및 원격 verify/네 signed image, 동일 SHA 개발 서버·공개 PC/mobile smoke로 재판정했다. BE-01~04, ES-01~07, UI-01~03/06/08, OPS-01 및 **적용 개발 경로의** OPS-02는 명시한 범위에서 해결/수용했다. UI-04/05는 중복 복제와 목록·오류 표시 결함이 재현된 **미해결 P2**로 1.2.5 P3, UI-07의 오류 읽기 위치는 **미해결 P2**로 1.2.1 P3에 남는다. OPS-03 Caddy 상한은 개발 경로에는 미적용이며 릴리스 proxy는 건드리지 않아 운영 인수 전 미검증이다. OPS-100-001 외부 backup/RPO 예외는 OPS-01 잠금 보정으로 자동 해소되지 않는다.

공개 edge에서 임의 XFF 변경 중에도 읽기/게스트 제한이 429가 되었고, 위조 `CF-Connecting-IP`는 Cloudflare가 앱 전에 403으로 거부했다. 이 두 결과는 다르며 릴리스 proxy 안전성까지 주장하지 않는다. 실제 Windows/iOS/Android·OS IME·AT는 사용자 지시로 미실행/후속 보류다. 위 P2 재현에서는 현재 자료 원문 손실·타 계정 인가 확대·복구 불능 증거가 없었으므로 필수 차단으로 승격하지 않았으나, 새 증거가 생기면 P5/후속 착수를 중단한다. 상세 CI·digest·DB·rollback·개발 배포와 synthetic 정리는 [P4 최종 인수](4phase.md)를 따른다.
