...
- [ ] `LC-RD-125-P2-01` 첫 조회와 load-more에 동일 generation 검사를 적용하고 abort만으로 안전하다고 가정하지 않는다. **[WC-07](../BRANCH-COMPARISON-118-P4.md)**의 b에 통합된 수정·회귀를 먼저 인수하고 미인수 잔여에만 추가 적용한다: 4화면 후보와 회귀를 선별한다. song-link-manager까지 확대하고 items/count/cursor/filterOptions/orderVersion/loading/error가 최신 query 소유인지 확인한다.
