...
|---|---|---|---|---|---|
| 2026-09-05 | 0.2.0 / 운영 보완 | Codex | 로컬·개발·릴리스 Docker 작업 뒤 LyricsCloud 중지 컨테이너·dangling image·미사용 network와 builder cache를 정리하고 volume·실행 중 자산은 보존하는 공통 스크립트와 운영 원칙을 추가 | shell·옵션·Compose 검사, check, 59 tests, dry-run, 로컬 실제 정리 6.848GB 회수 및 실행 중 컨테이너·전체 volume 전후 일치, [`정리 runbook`](../../docs/runbooks/docker-cleanup.md) | 개발 배포 성공 경로에서 자동 실행, 릴리스는 명시적으로 승인된 배포 smoke 성공 뒤 실행 |
