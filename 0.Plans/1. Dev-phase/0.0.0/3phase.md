...
- [ ] LC-000-P3-01 — Accepted 토폴로지를 반영한 최상위 모듈과 책임 설명을 작성한다.
- [ ] LC-000-P3-02 — domain에서 application, adapter, UI로 향하는 허용 의존 방향을 규칙으로 고정한다.
- [ ] LC-000-P3-03 — 15개 화면의 route, 공개·인증 필요 여부, 아직 사용할 수 없는 상태를 manifest에 정의한다.
- [ ] LC-000-P3-04 — 사용자 ID, resource ID, 명령 결과, validation 오류의 공통 타입을 정의한다.
- [ ] LC-000-P3-05 — 필수 환경 값 누락 시 시작 단계에서 이름만 알리고 값은 출력하지 않는 검증 계약을 만든다.
- [ ] LC-000-P3-06 — liveness와 readiness가 서로 다른 실패를 표현하도록 응답 계약을 정의한다.
- [ ] LC-000-P3-07 — PC 1440px와 모바일 390×844 기준의 레이아웃 토큰과 safe-area 규칙을 정리한다.
- [ ] LC-000-P3-08 — 특정 인프라 패키지가 도메인 경계를 넘어오는지 검사할 정적 규칙을 준비한다.
- [ ] LC-000-P3-09 — README·Agent.md·AGENTS.md·STATUS·CHANGELOG의 역할과 갱신 책임을 실제 파일과 대조한다.
- [ ] LC-000-P3-10 — `.gitignore`, PR template, Phase 작업 Issue template이 비밀·대용량 runtime 자료와 범위 밖 변경을 막는지 검토한다.
...
- [ ] 저장소 모듈 책임과 의존 방향이 문서화됐다.
- [ ] 15개 route가 빠짐없이 manifest에 등록됐다.
- [ ] 환경 설정과 health 계약이 검토됐다.
- [ ] PC·모바일 공통 토큰 골격이 정의됐다.
- [ ] 인프라 교체가 도메인 변경을 요구하지 않는 경계가 마련됐다.
- [ ] 사람과 Agent가 현재 Phase와 파일 소유 범위를 찾을 수 있는 협업 문서가 검증됐다.
- [ ] GitHub 협업 템플릿과 민감 파일 제외 규칙이 검증됐다.
