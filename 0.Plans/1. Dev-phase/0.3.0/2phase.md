...
- [ ] LC-030-P2-01 — CodeMirror 인스턴스를 route별 lyric ID와 연결하고 unmount 때 listener와 view를 해제한다.
- [ ] LC-030-P2-02 — editor state의 document 문자열만 저장하며 DOM, decoration, selection을 body에 포함하지 않는다.
- [ ] LC-030-P2-03 — initial body를 한 번 주입하고 데이터 재조회가 사용자의 미저장 입력을 덮지 않게 한다.
- [ ] LC-030-P2-04 — compositionstart부터 compositionend까지 한글 조합 중간 상태로 서버 저장을 확정하지 않는다.
- [ ] LC-030-P2-05 — 붙여넣기에서 서식 HTML을 버리고 줄바꿈을 포함한 plain text만 삽입한다.
- [ ] LC-030-P2-06 — undo·redo가 서버 재조회나 제목 입력 때문에 초기화되지 않도록 editor transaction 경계를 분리한다.
- [ ] LC-030-P2-07 — 마지막 확정 입력 후 짧은 지연으로 현재본 저장을 요청하고 같은 문서 요청을 순서대로 처리한다.
- [ ] LC-030-P2-08 — 저장 응답이 해당 요청 이후 추가 변경까지 저장된 것처럼 상태를 표시하지 않게 sequence를 비교한다.
- [ ] LC-030-P2-09 — PC에서는 읽기 좋은 최대 행 폭을, 모바일에서는 100dvh와 safe-area를 적용해 본문을 우선 표시한다.
- [ ] LC-030-P2-10 — 저장 실패 시 현재 editor 내용을 유지하고 명시적 재시도 행동을 제공한다.
...
- [ ] CodeMirror와 DB 사이에 순수 텍스트만 왕복한다.
- [ ] PC·모바일 기본 편집 화면이 같은 Phase에서 동작한다.
- [ ] 한글 IME, 붙여넣기, undo·redo에서 본문 손실이 없다.
- [ ] 단일 세션 저장 요청이 직렬화되고 상태가 정확하다.
- [ ] 실패 후 입력이 보존되고 재시도할 수 있다.
- [ ] CRDT·수정 기록을 완료 기능처럼 표시하지 않는다.
