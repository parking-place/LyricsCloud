...
- [ ] `LC-NF-1.0.8-P3-01` 곡 카드/리스트에 드래그 handle을 제공하되 텍스트 선택과 충돌시키지 않는다.
- [ ] `LC-NF-1.0.8-P3-02` 이동 시 사용자정렬로 전환됐음을 안내하고 새로고침 후 유지한다.
- [ ] `LC-NF-1.0.8-P3-03` 앞으로·뒤로·맨 앞/뒤 또는 위치 선택의 보이는 대안을 제공한다.
- [ ] `LC-NF-1.0.8-P3-04` 서버 실패 시 원래 순서를 복원하고 다시 시도하되 성공한 이동을 중복 적용하지 않는다.
- [ ] `LC-NF-1.0.8-P3-05` 제안 `tests/e2e/new-feature-1.0.8.spec.ts`에 실제 기능 경로를 등록하고 정상·빈 상태·실패·권한 없음·로딩을 PC와 모바일에서 검사한다. native이면 플랫폼 runner와 UI 테스트로 대체/병행한다.
- [ ] `LC-NF-1.0.8-P3-06` 입력·선택·IME·undo·로컬 초안·서버 저장 상태의 연속성을 확인한다. 정상 UI만 보여주기 위해 오류/권한 검사를 제거하지 않는다.
