...
- [ ] `LC-NF-1.0.10-P2-01` 구현 전 `1.0.10` 수용 사례의 실패 테스트를 작성한다. 제안 위치는 `tests/new-feature/1.0.10.contract.test.ts`이며 runner 포함 여부를 확인한 뒤 실패 이유를 기록한다.
- [ ] `LC-NF-1.0.10-P2-02` 곡 모델명·작업 링크의 owner/parent 제약과 제한 수량·길이를 추가한다.
- [ ] `LC-NF-1.0.10-P2-03` URL protocol/승인된 host/path를 검증하고 javascript/data/내부 주소 등을 거부한다.
- [ ] `LC-NF-1.0.10-P2-04` 링크 생성·수정·제거를 멱등 처리하고 곡 soft delete·복원·내보내기와 연결한다.
- [ ] `LC-NF-1.0.10-P2-05` 외부 자동 조회 없이 URL·선택 모델명을 안전하게 저장하는 최소 기능을 완성한다.
- [ ] `LC-NF-1.0.10-P2-06` 추가 schema가 있으면 실제 테스트 DB의 빈 설치·이전 schema 업그레이드·권한·되돌림을 검사한다. 원인 수정 후 동일 실패 테스트와 기존 관련 회귀를 다시 실행한다.
...
[1.0.10 P3](3phase.md)에 입력·실행 결과·호환 및 중단 조건을 넘긴다. 다음 단계의 승인이 없거나 선행 증거가 부족하면 자동 진행하지 않는다. 문서 작성으로 runtime version을 바꾸지 않는다.
