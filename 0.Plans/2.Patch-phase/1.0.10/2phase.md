# 1.0.10 Phase 2 — 핵심 기반·저장과 서버

- 상태: **완료** (`complete`, 2026-09-11)
- 단계 목적: Suno 모델명·복수 작업 링크의 핵심 기반·저장과 서버을 완료하고 다음 단계에 검증 가능한 입력을 전달한다.
...
- [1.0.10 P1](1phase.md)의 산출물·검증/승인과 실제 최종 SHA 인수.
...
- [x] `LC-NF-1.0.10-P2-01` 구현 전 `1.0.10` 수용 사례의 실패 테스트를 작성한다. 제안 위치는 `tests/new-feature/1.0.10.contract.test.ts`이며 runner 포함 여부를 확인한 뒤 실패 이유를 기록한다.
- [x] `LC-NF-1.0.10-P2-02` 곡 모델명·작업 링크의 owner/parent 제약과 제한 수량·길이를 추가한다.
- [x] `LC-NF-1.0.10-P2-03` URL protocol/승인된 host/path를 검증하고 javascript/data/내부 주소 등을 거부한다.
- [x] `LC-NF-1.0.10-P2-04` 링크 생성·수정·제거를 멱등 처리하고 곡 soft delete·복원·내보내기와 연결한다.
- [x] `LC-NF-1.0.10-P2-05` 외부 자동 조회 없이 URL·선택 모델명을 안전하게 저장하는 최소 기능을 완성한다.
- [x] `LC-NF-1.0.10-P2-06` 추가 schema가 있으면 실제 테스트 DB의 빈 설치·이전 schema 업그레이드·권한·되돌림을 검사한다. 원인 수정 후 동일 실패 테스트와 기존 관련 회귀를 다시 실행한다.
...
- 1.0.10 P2의 검토 가능한 변경/계약과 수용 사례 증거.
- 변경 파일·추가 migration·실행 명령/환경·결과·미실행/잔여 위험의 인수 기록.
- 후속 소비자가 유지할 API·데이터·copy·권한·UI 상태 계약.

## 다음 Phase 인계

[P2 인수 기록](../../../docs/runbooks/1.0.10-phase2-suno-workspace-store.md)의 domain parser, `1003_song_suno_workspaces.sql`, owner RLS·aggregate CAS/API·export와 공개 개발 API smoke를 [1.0.10 P3](3phase.md)에 넘긴다. P3는 이 계약을 바꾸지 않고 PC·모바일 모델/링크 UI, 새 탭 보호, draft·오류 복구를 완성한다.
