...
- [ ] `LC-NF-1.0.10-P4-01` `AC-1.0.10-01`: 한 곡에 서로 다른 3개 Suno 링크 상황을 재현하여 “각 링크가 독립적으로 저장·재접속·새 탭 열기 된다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.0.10-P4-02` `AC-1.0.10-02`: 다른 계정의 곡 ID에 링크 추가 상황을 재현하여 “거부하며 링크/모델 정보가 노출되지 않는다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.0.10-P4-03` `AC-1.0.10-03`: 링크 제거/곡 휴지통/복원 상황을 재현하여 “외부 원곡은 건드리지 않고 내부 데이터 계약대로 복원된다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.0.10-P4-04` `AC-1.0.10-04`: 알 수 없는 모델명·잘못된 URL 상황을 재현하여 “문법/길이 정책대로 처리하고 원문과 다른 링크는 유지한다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.0.10-P4-05` 직전 버전·다른 계정·오프라인·재접속·서버 재시작·지원 브라우저/기기 회귀를 수행하고 미실행/skip의 원인을 기록한다.
- [ ] `LC-NF-1.0.10-P4-06` 발견된 원인을 최소 수정 후 같은 재현을 다시 실행한다. 성능 변경은 동일 입력·환경의 전후 값과 결과 동등성을 증명하고 테스트 삭제로 통과시키지 않는다.
...
[1.0.10 P5](5phase.md)에 입력·실행 결과·호환 및 중단 조건을 넘긴다. 다음 단계의 승인이 없거나 선행 증거가 부족하면 자동 진행하지 않는다. 문서 작성으로 runtime version을 바꾸지 않는다.
