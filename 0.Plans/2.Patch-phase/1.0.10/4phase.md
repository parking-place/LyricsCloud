# 1.0.10 Phase 4 — 실패·권한·복구 회귀

- 상태: **완료** (`complete`, 2026-09-11)
- 단계 목적: Suno 모델명·복수 작업 링크의 실패·권한·복구 회귀을 완료하고 다음 단계에 검증 가능한 입력을 전달한다.
...
- [1.0.10 P3](3phase.md)의 산출물·검증/승인과 실제 최종 SHA 인수.
...
- [x] `LC-NF-1.0.10-P4-01` `AC-1.0.10-01`: 한 곡에 서로 다른 3개 Suno 링크 상황을 재현하여 “각 링크가 독립적으로 저장·재접속·새 탭 열기 된다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [x] `LC-NF-1.0.10-P4-02` `AC-1.0.10-02`: 다른 계정의 곡 ID에 링크 추가 상황을 재현하여 “거부하며 링크/모델 정보가 노출되지 않는다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [x] `LC-NF-1.0.10-P4-03` `AC-1.0.10-03`: 링크 제거/곡 휴지통/복원 상황을 재현하여 “외부 원곡은 건드리지 않고 내부 데이터 계약대로 복원된다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [x] `LC-NF-1.0.10-P4-04` `AC-1.0.10-04`: 알 수 없는 모델명·잘못된 URL 상황을 재현하여 “문법/길이 정책대로 처리하고 원문과 다른 링크는 유지한다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [x] `LC-NF-1.0.10-P4-05` 직전 버전·다른 계정·오프라인·재접속·서버 재시작·지원 브라우저/기기 회귀를 수행하고 미실행/skip의 원인을 기록한다.
- [x] `LC-NF-1.0.10-P4-06` 발견된 원인을 최소 수정 후 같은 재현을 다시 실행한다. 성능 변경은 동일 입력·환경의 전후 값과 결과 동등성을 증명하고 테스트 삭제로 통과시키지 않는다.
...
- 1.0.10 P4의 검토 가능한 변경/계약과 수용 사례 증거.
- 변경 파일·추가 migration·실행 명령/환경·결과·미실행/잔여 위험의 인수 기록.
- 후속 소비자가 유지할 API·데이터·copy·권한·UI 상태 계약.

## 다음 Phase 인계

[P4 교차 회귀 기록](../../../docs/runbooks/1.0.10-phase4-suno-regression.md)의 실제 DB·지원 browser·CI·개발 재시작 결과를 [1.0.10 P5](5phase.md)에 넘긴다. 실제 Windows/iOS/Android 물리 기기는 자동 증거로 대체하지 않았고 백업 `OPS-100-001`은 사용자 승인 예외로 유지한다.
