...
- [ ] `LC-NF-1.0.10-P3-01` 모델 선택 UI에 미지정·예시 선택·필요한 사용자 지정/신규 값 처리를 승인 범위로 제공한다.
- [ ] `LC-NF-1.0.10-P3-02` 곡 대시보드에 여러 링크 버튼을 추가·이름 수정·제거할 수 있게 한다.
- [ ] `LC-NF-1.0.10-P3-03` 새 탭 열기에 opener 보호를 적용하고 링크 제거는 Suno 원본 삭제가 아님을 표시한다.
- [ ] `LC-NF-1.0.10-P3-04` 기본 카드에는 수동 제목/URL 안내를 제공하며 자동 썸네일·시간은 아직 없는 값으로 표시한다.
- [ ] `LC-NF-1.0.10-P3-05` 제안 `tests/e2e/new-feature-1.0.10.spec.ts`에 실제 기능 경로를 등록하고 정상·빈 상태·실패·권한 없음·로딩을 PC와 모바일에서 검사한다. native이면 플랫폼 runner와 UI 테스트로 대체/병행한다.
- [ ] `LC-NF-1.0.10-P3-06` 입력·선택·IME·undo·로컬 초안·서버 저장 상태의 연속성을 확인한다. 정상 UI만 보여주기 위해 오류/권한 검사를 제거하지 않는다.
...
[1.0.10 P4](4phase.md)에 입력·실행 결과·호환 및 중단 조건을 넘긴다. 다음 단계의 승인이 없거나 선행 증거가 부족하면 자동 진행하지 않는다. 문서 작성으로 runtime version을 바꾸지 않는다.
