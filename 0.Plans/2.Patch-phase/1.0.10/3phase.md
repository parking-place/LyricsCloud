# 1.0.10 Phase 3 — PC·모바일 사용자 흐름

- 상태: **완료** (`complete`, 2026-09-11)
- 단계 목적: Suno 모델명·복수 작업 링크의 PC·모바일 사용자 흐름을 완료하고 다음 단계에 검증 가능한 입력을 전달한다.
...
- [1.0.10 P2](2phase.md)의 산출물·검증/승인과 실제 최종 SHA 인수.
...
- [x] `LC-NF-1.0.10-P3-01` 모델 선택 UI에 미지정·예시 선택·필요한 사용자 지정/신규 값 처리를 승인 범위로 제공한다.
- [x] `LC-NF-1.0.10-P3-02` 곡 대시보드에 여러 링크 버튼을 추가·이름 수정·제거할 수 있게 한다.
- [x] `LC-NF-1.0.10-P3-03` 새 탭 열기에 opener 보호를 적용하고 링크 제거는 Suno 원본 삭제가 아님을 표시한다.
- [x] `LC-NF-1.0.10-P3-04` 기본 카드에는 수동 제목/URL 안내를 제공하며 자동 썸네일·시간은 아직 없는 값으로 표시한다.
- [x] `LC-NF-1.0.10-P3-05` 제안 `tests/e2e/new-feature-1.0.10.spec.ts`에 실제 기능 경로를 등록하고 정상·빈 상태·실패·권한 없음·로딩을 PC와 모바일에서 검사한다. native이면 플랫폼 runner와 UI 테스트로 대체/병행한다.
- [x] `LC-NF-1.0.10-P3-06` 입력·선택·IME·undo·로컬 초안·서버 저장 상태의 연속성을 확인한다. 정상 UI만 보여주기 위해 오류/권한 검사를 제거하지 않는다.
...
- 1.0.10 P3의 검토 가능한 변경/계약과 수용 사례 증거.
- 변경 파일·추가 migration·실행 명령/환경·결과·미실행/잔여 위험의 인수 기록.
- 후속 소비자가 유지할 API·데이터·copy·권한·UI 상태 계약.

## 다음 Phase 인계

[P3 UI 인수 기록](../../../docs/runbooks/1.0.10-phase3-suno-workspace-ui.md)의 model selector, 링크 편집·정렬·제거, 새 탭 보호, 브라우저 탭 초안·오류 복구와 desktop/mobile 증거를 [1.0.10 P4](4phase.md)에 넘긴다. P4는 같은 계약에서 soft-delete 복원·owner·offline/reconnect·재시작·지원 엔진 교차 회귀를 수행한다.
