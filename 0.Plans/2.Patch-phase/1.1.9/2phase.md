...
- 단계 목적: Windows 작성·오프라인 복구의 핵심 기반·저장과 서버을 완료하고 다음 단계에 검증 가능한 입력을 전달한다.
...
- [1.1.9 P1](1phase.md)의 산출물·검증/승인과 실제 최종 SHA 인수.
...
- [ ] `LC-NF-1.1.9-P2-01` 구현 전 `1.1.9` 수용 사례의 실패 테스트를 작성한다. 제안 위치는 `tests/new-feature/1.1.9.contract.test.ts`이며 runner 포함 여부를 확인한 뒤 실패 이유를 기록한다.
- [ ] `LC-NF-1.1.9-P2-02` owner/document별 crash-safe local store와 멱등 outbox를 구현한다.
- [ ] `LC-NF-1.1.9-P2-03` 원자 ACK·재접속·서버 재시작·거부된 update를 웹과 동일 계약으로 처리한다.
- [ ] `LC-NF-1.1.9-P2-04` OS 종료·절전·앱 업데이트·키 유실에서 보존 가능한 초안과 한계를 구분한다.
- [ ] `LC-NF-1.1.9-P2-05` private/selected/public write를 플랫폼에서도 동일 검사하고 clipboard/export 직렬화를 맞춘다.
- [ ] `LC-NF-1.1.9-P2-06` 추가 schema가 있으면 실제 테스트 DB의 빈 설치·이전 schema 업그레이드·권한·되돌림을 검사한다. 원인 수정 후 동일 실패 테스트와 기존 관련 회귀를 다시 실행한다.
...
- 1.1.9 P2의 검토 가능한 변경/계약과 수용 사례 증거.
- 변경 파일·추가 migration·실행 명령/환경·결과·미실행/잔여 위험의 인수 기록.
- 후속 소비자가 유지할 API·데이터·copy·권한·UI 상태 계약.

## 다음 Phase 인계

[1.1.9 P3](3phase.md)에 입력·실행 결과·호환 및 중단 조건을 넘긴다. 다음 단계의 승인이 없거나 선행 증거가 부족하면 자동 진행하지 않는다. 문서 작성으로 runtime version을 바꾸지 않는다.
