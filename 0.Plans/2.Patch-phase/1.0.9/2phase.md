...
- [ ] `LC-NF-1.0.9-P2-01` 구현 전 `1.0.9` 수용 사례의 실패 테스트를 작성한다. 제안 위치는 `tests/new-feature/1.0.9.contract.test.ts`이며 runner 포함 여부를 확인한 뒤 실패 이유를 기록한다.
- [ ] `LC-NF-1.0.9-P2-02` 공통 이동 명령을 자료 유형별 검증으로 확장하되 무분별한 공통화를 피한다.
- [ ] `LC-NF-1.0.9-P2-03` 프롬프트 카드 이동은 prompt_tokens occurrence/순서를 수정하지 않게 한다.
- [ ] `LC-NF-1.0.9-P2-04` 삭제·복원·복제·새 자료 추가의 기본 위치와 rank 충돌을 검증한다.
- [ ] `LC-NF-1.0.9-P2-05` 검색·favorite·연결 곡 필터·키셋에 적용되는 순서 계약을 테스트한다.
- [ ] `LC-NF-1.0.9-P2-06` 추가 schema가 있으면 실제 테스트 DB의 빈 설치·이전 schema 업그레이드·권한·되돌림을 검사한다. 원인 수정 후 동일 실패 테스트와 기존 관련 회귀를 다시 실행한다.
