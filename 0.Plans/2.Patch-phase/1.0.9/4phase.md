...
- [ ] `LC-NF-1.0.9-P4-01` `AC-1.0.9-01`: 프롬프트 카드를 이동한 뒤 전체 복사 상황을 재현하여 “자료 카드 순서만 바뀌고 토큰 순서와 출력은 같다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.0.9-P4-02` `AC-1.0.9-02`: 라임 목록의 길게 누르기와 드래그 상황을 재현하여 “복사와 이동이 중복 실행되지 않는다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.0.9-P4-03` `AC-1.0.9-03`: 즐겨찾기 필터에서 삭제→복원 상황을 재현하여 “자료가 중복되지 않고 계약한 개인 위치로 돌아온다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.0.9-P4-04` `AC-1.0.9-04`: 세 자료 유형 순서를 연속 변경 상황을 재현하여 “각 유형의 순서와 전역 핀이 독립적으로 유지된다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.0.9-P4-05` 직전 버전·다른 계정·오프라인·재접속·서버 재시작·지원 브라우저/기기 회귀를 수행하고 미실행/skip의 원인을 기록한다.
- [ ] `LC-NF-1.0.9-P4-06` 발견된 원인을 최소 수정 후 같은 재현을 다시 실행한다. 성능 변경은 동일 입력·환경의 전후 값과 결과 동등성을 증명하고 테스트 삭제로 통과시키지 않는다.
