...
- [x] `LC-NF-1.0.9-P5-01` 1.0.9의 요구 범위와 각 수용 사례를 정확한 source SHA·변경 파일·migration·실행 명령·결과에 연결한다.
- [x] `LC-NF-1.0.9-P5-02` 사용자 가이드·README·지원·보안·자가호스팅·현재 계획/결정 문서를 라임·프롬프트 사용자정렬의 실제 동작에 맞춰 갱신하고 과거 검증 기록은 보존한다.
- [x] `LC-NF-1.0.9-P5-03` 원격 CI와 허용된 개발 산출물·동일 SHA의 개발 환경을 확인한다. 문서-only 결과면 문서 승인만 기록하고 앱 배포 성공을 주장하지 않는다.
- [x] `LC-NF-1.0.9-P5-04` 확인된 P0/P1, 원문 유실, 인증 우회, 무음 저장 실패가 없음을 실제 범위에서 판정한다. 신규 범위의 필수 안전성을 다음 안정화 패치로 미루지 않는다.
- [x] `LC-NF-1.0.9-P5-05` 다섯 Phase 후보 인수 이후에만 별도 release go/no-go를 요청할 수 있다. main 병합·정식 별칭·app.example.test 배포는 현재 승인 범위를 확인하고 실행한다.
- [x] `LC-NF-1.0.9-P5-06` 후속 패치에 계약·미해결/보류·실기기/운영 증거·중단/복구 조건을 인계한다. 같은 제품 계열을 유지하고 빈 릴리스·자동 minor 승격을 만들지 않는다.
...
후보 `d0dff173995298a4d617d7494b7c2cc9f32bd542`는 Actions `34583462005`의 전체 verify와 web·collaboration·migrate·worker dev image 게시·서명을 통과했다. 같은 SHA를 개발 서버에 배포해 `1.0.9`·`dev`·`p5`, schema `1002_library_manual_order.sql`, 네 서비스 healthy를 확인했다. 공개 desktop/mobile에서 라임·프롬프트 이동·재진입, prompt 원문 불변, touch copy 분리, 좁은 화면 overflow 없음과 web·collaboration·worker 재시작 뒤 순서 지속성이 PASS했고 합성 자료를 제거했다. 신규 제품 P0/P1·원문 유실·인증 우회·무음 저장 실패는 0건이며 실제 물리 기기 미실행과 `OPS-100-001` 예외는 유지한다.
