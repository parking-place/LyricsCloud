...
- [ ] `LC-NF-1.0.9-P3-01` 라임·프롬프트의 리스트와 모든 그리드 크기에 동일 이동 수단을 제공한다.
- [ ] `LC-NF-1.0.9-P3-02` 모바일 길게 눌러 복사와 드래그 제스처의 시작 조건을 분리한다.
- [ ] `LC-NF-1.0.9-P3-03` 이동 전후 focus와 화면 읽기 알림을 자료 제목의 불필요한 로그 없이 전달한다.
- [ ] `LC-NF-1.0.9-P3-04` 서버/오프라인 오류 때 순서가 저장되지 않았음을 안내하고 원문 편집은 계속 보존한다.
- [ ] `LC-NF-1.0.9-P3-05` 제안 `tests/e2e/new-feature-1.0.9.spec.ts`에 실제 기능 경로를 등록하고 정상·빈 상태·실패·권한 없음·로딩을 PC와 모바일에서 검사한다. native이면 플랫폼 runner와 UI 테스트로 대체/병행한다.
- [ ] `LC-NF-1.0.9-P3-06` 입력·선택·IME·undo·로컬 초안·서버 저장 상태의 연속성을 확인한다. 정상 UI만 보여주기 위해 오류/권한 검사를 제거하지 않는다.
