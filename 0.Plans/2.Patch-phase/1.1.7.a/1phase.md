...
- [ ] `LC-PLAN-117A-P1-06`: current `1.1.8 P4 review`와 계획된 1.1.9~1.1.14의 선행 관계를 대조해 숫자 patch 재배정안·변경 파일 담당·API/DB 호환·롤백·실기기 미실행 gate를 승인받는다. 이 승인 전 `1.1.7.a`를 runtime version으로 사용하지 않는다.

## 수용·인계

`AC-117A-01~05`별 정상/실패/권한/복구 입력표, 확정 클릭 대상, 사진 저장·공개·삭제 결정, OAuth override 우선순위, 현재 source SHA, 예정 숫자 patch와 선행조건이 한 문서로 연결돼야 한다. 임시 URL-only UI나 OAuth 덮어쓰기 유지안을 승인된 설계로 간주하지 않는다. P1 설계 검토는 앱 테스트/배포 PASS가 아니며 [P2](2phase.md)는 확정 계약 뒤에만 시작한다.
