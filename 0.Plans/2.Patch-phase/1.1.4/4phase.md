...
- [ ] `LC-NF-1.1.4-P4-01` `AC-1.1.4-01`: 기기 절전·네트워크 전환·서버 재시작 반복 상황을 재현하여 “원문·권한·영수증이 수렴하고 대기 queue가 무한 증가하지 않는다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.1.4-P4-02` `AC-1.1.4-02`: owner가 복원 중 offline writer 재접속 상황을 재현하여 “승인된 복원/병합 정책과 서로의 원문 보존을 확인한다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.1.4-P4-03` `AC-1.1.4-03`: 권한 회수 후 다른 계정 로그인 상황을 재현하여 “이전 actor의 자료·초안이 다음 계정에 노출되지 않는다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.1.4-P4-04` `AC-1.1.4-04`: 가속 소크만 실행한 경우 상황을 재현하여 “실제 장시간·실기기 검증을 통과했다고 기록하지 않는다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.1.4-P4-05` 직전 버전·다른 계정·오프라인·재접속·서버 재시작·지원 브라우저/기기 회귀를 수행하고 미실행/skip의 원인을 기록한다.
- [ ] `LC-NF-1.1.4-P4-06` 발견된 원인을 최소 수정 후 같은 재현을 다시 실행한다. 성능 변경은 동일 입력·환경의 전후 값과 결과 동등성을 증명하고 테스트 삭제로 통과시키지 않는다.

- [ ] `LC-NF-1.1.4-P4-07` 사람 수·현재 작업 위치·철회/재접속의 전체 협업 인수를 3계정·2기기 이상에서 확인하고 기존 same-owner 탭 인수와 따로 기록한다.
