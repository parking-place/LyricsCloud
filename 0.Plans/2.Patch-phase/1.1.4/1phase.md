...
- [ ] `LC-NF-1.1.4-P1-01` 이전 공유 패치의 기본 회수·인가·초안 보호는 출시 필수이며 이 단계까지 미루지 않는다.
- [ ] `LC-NF-1.1.4-P1-02` 소유자 복원·삭제·동시 writer 재접속의 기대 상태를 사건 순서로 정의한다.
- [ ] `LC-NF-1.1.4-P1-03` 자동화된 빠른 반복 시험과 실제 장시간 브라우저·절전·복귀 증거를 구분한다.
- [ ] `LC-NF-1.1.4-P1-04` 관련 구현·테스트 경로를 읽고 위 요구/수용 사례를 정상·오류·권한·복구 입력표로 고정한다. 원인 미확정 항목은 가설과 증거 수준을 표시한다.
- [ ] `LC-NF-1.1.4-P1-05` 저장 형식·API·순서·copy·권한 중 변경되는 인터페이스와 호환/되돌림을 결정 문서에 기록하고 최초 소비 전에 승인한다.
- [ ] `LC-NF-1.1.4-P1-06` 각 변경 파일 담당자와 실제 최종 source SHA를 기록한다. 해당 패치 밖의 기능이나 아직 선택하지 않은 아이디어를 섞지 않는다.
