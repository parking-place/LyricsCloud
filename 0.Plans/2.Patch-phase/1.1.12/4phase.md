...
- [ ] `LC-NF-1.1.12-P4-01` 명시 승인과 P2 성공 후에만 Linux의 승인 범위 읽기·작성·검색·copy를 기존 API/원문 형식으로 구현한다.
- [ ] `LC-NF-1.1.12-P4-02` 협업 subscription/철회·offline outbox/복구·사전 제공자 실패·폰트 fallback을 OS 수명주기에 연결한다.
- [ ] `LC-NF-1.1.12-P4-03` 설정·로그아웃·오류/재연결·권한없음 상태를 만들고 승인된 패키지 업데이트 실패 때 기존 앱/초안을 복원한다.
