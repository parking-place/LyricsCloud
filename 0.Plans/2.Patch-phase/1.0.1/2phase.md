...
상태: **진행 중**. P1의 승인된 베타 코드·grant·receipt 계약과 환경 격리 기준을 인수했다.
...
- [ ] `LC-NF-1.0.1-P2-01` 베타코드 상태·가입 intent·접근 grant·소비 영수증을 추가 migration으로 구현한다. 앱 계정 상태와 접근 grant를 분리하고 blocked/withdrawn 계정의 재가입 우회를 차단한다.
- [ ] `LC-NF-1.0.1-P2-02` 호스트에서 LyricsCloud betacode -n 7을 실행하면 CSPRNG 기반 A-Z0-9 6자리 7개가 생성되게 한다. 중복은 DB 유일 제약과 제한 재시도로 처리하고 used/revoked 코드는 재발급하지 않는다.
- [ ] `LC-NF-1.0.1-P2-03` LyricsCloud betacode ls는 관리자에게 현재 사용 가능한 코드를 보여준다. 미사용 코드만 인증 암호화로 복구하고 웹 런타임에는 복호화 키·관리 권한을 주지 않는다.
- [ ] `LC-NF-1.0.1-P2-04` LyricsCloud betacode refresh는 확인된 환경의 미사용·가입 대기 코드를 원자적으로 폐기한다. 기존 가입 grant·사용 이력·사용자 데이터는 변경하지 않는다.
- [ ] `LC-NF-1.0.1-P2-05` 일괄 발급·조회·refresh와 가입 엔드포인트에 최소 권한·출력 비밀성·시간 제한·재시작에도 유지되는 남용 제한을 연결한다. CLI 설치 wrapper와 컨테이너 실행 경로를 함께 문서화한다.
