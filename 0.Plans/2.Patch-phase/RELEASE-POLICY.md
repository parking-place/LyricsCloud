...
2026-09-16 **`1.1.7a` 단일 예외**: 사용자는 미완료 1.1.8 Windows P3가 들어간 현행 main을 릴리스에 섞지 않도록 `v1.1.7` 출시 SHA에서 만든 `release/1.1.7a` 전용 브랜치에 P1~P5 PR을 순차 통합하고, 해당 최종 SHA의 필수 CI·개발 인수 뒤 그 브랜치 SHA에 annotated `v1.1.7a`를 붙여 정식 image/운영 배포하는 추가 예외를 승인했다. main에는 직접 push/병합하지 않으며 기존 `v1.1.7` tag·이미지·DB 볼륨을 이동/삭제하지 않는다. 이 예외는 다른 버전의 main/릴리스 절차나 1.1.8 Phase gate를 변경하지 않는다.
