...
- [ ] `LC-NF-1.0.4-P1-01` U+002E 마침표를 포함하는 lossless span으로 표시하며 span 연결이 정확히 raw가 되게 한다.
- [ ] `LC-NF-1.0.4-P1-02` 1,000자는 최종 복사 payload의 Unicode code point 수로 정하고 공백·줄바꿈도 포함한다.
- [ ] `LC-NF-1.0.4-P1-03` 경고는 초과 시 안내만 한다. 입력·저장·복사를 차단하거나 자동으로 줄이지 않는다.
- [ ] `LC-NF-1.0.4-P1-04` 관련 구현·테스트 경로를 읽고 위 요구/수용 사례를 정상·오류·권한·복구 입력표로 고정한다. 원인 미확정 항목은 가설과 증거 수준을 표시한다.
- [ ] `LC-NF-1.0.4-P1-05` 저장 형식·API·순서·copy·권한 중 변경되는 인터페이스와 호환/되돌림을 결정 문서에 기록하고 최초 소비 전에 승인한다.
- [ ] `LC-NF-1.0.4-P1-06` 각 변경 파일 담당자와 실제 최종 source SHA를 기록한다. 해당 패치 밖의 기능이나 아직 선택하지 않은 아이디어를 섞지 않는다.
