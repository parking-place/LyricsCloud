...
- [ ] `LC-NF-1.0.4-P2-01` 구현 전 `1.0.4` 수용 사례의 실패 테스트를 작성한다. 제안 위치는 `tests/new-feature/1.0.4.contract.test.ts`이며 runner 포함 여부를 확인한 뒤 실패 이유를 기록한다.
- [ ] `LC-NF-1.0.4-P2-02` 마침표·연속점·끝 미완성 구간·CRLF를 보존하는 span parser와 inverse 테스트를 만든다.
- [ ] `LC-NF-1.0.4-P2-03` 서버/목록/패널이 실제 복사할 문자열을 기준으로 길이와 경고 상태를 계산한다.
- [ ] `LC-NF-1.0.4-P2-04` mode 변경·다른 탭 업데이트 후 표시 span이 최신 raw의 같은 버전을 사용하게 한다.
- [ ] `LC-NF-1.0.4-P2-05` 경고 계산은 composition 중간 값을 저장하지 않으며 장문 입력에서 반복 전체 처리를 측정한다.
- [ ] `LC-NF-1.0.4-P2-06` 추가 schema가 있으면 실제 테스트 DB의 빈 설치·이전 schema 업그레이드·권한·되돌림을 검사한다. 원인 수정 후 동일 실패 테스트와 기존 관련 회귀를 다시 실행한다.
