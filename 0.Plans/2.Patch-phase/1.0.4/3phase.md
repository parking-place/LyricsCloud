...
- [ ] `LC-NF-1.0.4-P3-01` 문장형을 마침표 기준 구간으로 보여주되 장문의 원문 편집 진입을 유지한다.
- [ ] `LC-NF-1.0.4-P3-02` 태그형·문장형에 1,000자 초과 상태와 실제 길이를 일관되게 표시한다.
- [ ] `LC-NF-1.0.4-P3-03` 목록 빠른 복사·길게 누르기·가사 자료 패널·수동 복사 대안에도 같은 경고를 연결한다.
- [ ] `LC-NF-1.0.4-P3-04` 접근성 알림은 매 키 입력마다 중복 낭독하지 않고 초과 상태가 변할 때 명료하게 전달한다.
- [ ] `LC-NF-1.0.4-P3-05` 제안 `tests/e2e/new-feature-1.0.4.spec.ts`에 실제 기능 경로를 등록하고 정상·빈 상태·실패·권한 없음·로딩을 PC와 모바일에서 검사한다. native이면 플랫폼 runner와 UI 테스트로 대체/병행한다.
- [ ] `LC-NF-1.0.4-P3-06` 입력·선택·IME·undo·로컬 초안·서버 저장 상태의 연속성을 확인한다. 정상 UI만 보여주기 위해 오류/권한 검사를 제거하지 않는다.
