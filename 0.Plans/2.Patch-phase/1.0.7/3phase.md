...
- [ ] `LC-NF-1.0.7-P3-01` 세 목록에 리스트·작은·중간·큰 그리드의 선택 UI를 일관되게 제공한다.
- [ ] `LC-NF-1.0.7-P3-02` 제목·미리보기·수정일·액션의 정보 우선순위를 각 밀도에 맞춘다.
- [ ] `LC-NF-1.0.7-P3-03` 키보드/스크린리더로 선택 상태를 알 수 있고 줄바꿈/확대에서 겹치지 않게 한다.
- [ ] `LC-NF-1.0.7-P3-04` 새로고침·계정 전환·새 기기에서 저장 범위에 맞는 설정을 복원한다.
- [ ] `LC-NF-1.0.7-P3-05` 제안 `tests/e2e/new-feature-1.0.7.spec.ts`에 실제 기능 경로를 등록하고 정상·빈 상태·실패·권한 없음·로딩을 PC와 모바일에서 검사한다. native이면 플랫폼 runner와 UI 테스트로 대체/병행한다.
- [ ] `LC-NF-1.0.7-P3-06` 입력·선택·IME·undo·로컬 초안·서버 저장 상태의 연속성을 확인한다. 정상 UI만 보여주기 위해 오류/권한 검사를 제거하지 않는다.
