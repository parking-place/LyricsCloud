# 1.0.7 Phase 3 — PC·모바일 사용자 흐름

- 상태: **완료** (`complete`, 2026-09-11)
- 단계 목적: 목록 보기 선택·밀도 설정의 PC·모바일 사용자 흐름을 완료하고 다음 단계에 검증 가능한 입력을 전달한다.
...
- [1.0.7 P2](2phase.md)의 산출물·검증/승인과 실제 최종 SHA 인수.
...
- [x] `LC-NF-1.0.7-P3-01` 세 목록에 공통 selector와 리스트·작은·중간·큰 grid class를 연결했다.
- [x] `LC-NF-1.0.7-P3-02` 작은 밀도는 보조 미리보기만 감추고 제목·수정일·액션을 유지하며 중/대 밀도에 단계별 최소 높이를 적용했다.
- [x] `LC-NF-1.0.7-P3-03` group/aria-pressed/aria-busy, 키보드 실행, 320px·200% 확대·긴 한글·수평 overflow를 desktop/mobile에서 확인했다.
- [x] `LC-NF-1.0.7-P3-04` 새로고침·동일 계정 새 browser context·다른 계정·자료 유형별 복원을 실제 DB browser에서 확인했다.
- [x] `LC-NF-1.0.7-P3-05` `tests/e2e/new-feature-1.0.7.spec.ts` 정상·빈 목록 허용·저장 실패·권한 없음·로딩을 포함한 10건을 PC와 모바일에서 통과했다.
- [x] `LC-NF-1.0.7-P3-06` 보기 배열/key 불변과 기존 가입/가사·라임·프롬프트 저장·한글 composition·초안·두 탭 투영 30건을 PC/mobile에서 재통과했다.
...
- [x] 작업 ID마다 코드·실행 증거와 후보 SHA `7b824cacac53ef10164c8d6a92d80d2ce8e871c1`가 [P3 인수 기록](../../../docs/runbooks/1.0.7-phase3-library-view-ui.md)에 연결되어 있다.
- [x] item/query 불변·401 권한·저장 실패 rollback·stale 최신값 재조회와 정상 저장을 함께 구현/검증했다.
- [x] 실제 물리 기기 미실행과 중간 CI `[skip ci]`, P4 지원 browser/재시작 회귀를 숨김없이 기록했다.
- [x] 현재 상태/담당/변경 파일·관련 문서가 실제 수행 내용과 일치한다.
- [x] 로컬/실제 DB browser, 원격 branch/PR, 후보와 동일 SHA 개발 배포·공개 desktop/mobile 인수를 갖췄다.
- [x] P3에서 main·Release·릴리스 서버를 변경하지 않았다.

## 산출물

- 1.0.7 P3의 검토 가능한 변경/계약과 수용 사례 증거.
- 변경 파일·추가 migration·실행 명령/환경·결과·미실행/잔여 위험의 인수 기록.
- 후속 소비자가 유지할 API·데이터·copy·권한·UI 상태 계약.

## 다음 Phase 인계

[1.0.7 P4](4phase.md)에 입력·실행 결과·호환 및 중단 조건을 넘긴다. 다음 단계의 승인이 없거나 선행 증거가 부족하면 자동 진행하지 않는다. 문서 작성으로 runtime version을 바꾸지 않는다.

P4는 후보 SHA의 전체 실제 PostgreSQL 회귀, stale/오프라인/재접속·서버 재시작과 지원 browser matrix를 수행하고, 글꼴 설정 동시 갱신이 독립 CAS로 보존되는지 다시 확인한다.
