...
- [ ] `LC-NF-1.0.7-P4-01` `AC-1.0.7-01`: 검색·필터·현재 페이지 상태에서 보기 4종 전환 상황을 재현하여 “같은 자료 집합과 순서를 유지한다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.0.7-P4-02` `AC-1.0.7-02`: 계정 A는 리스트, B는 큰 그리드 상황을 재현하여 “계정 전환으로 A의 설정이 B에 새지 않는다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.0.7-P4-03` `AC-1.0.7-03`: 320px·200% 확대·긴 한글 제목 상황을 재현하여 “액션과 내용이 겹치지 않고 키보드 이동이 가능하다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.0.7-P4-04` `AC-1.0.7-04`: 다른 탭이 글꼴 설정 갱신 중 밀도 저장 상황을 재현하여 “무관한 글꼴 설정을 되돌리지 않는다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.0.7-P4-05` 직전 버전·다른 계정·오프라인·재접속·서버 재시작·지원 브라우저/기기 회귀를 수행하고 미실행/skip의 원인을 기록한다.
- [ ] `LC-NF-1.0.7-P4-06` 발견된 원인을 최소 수정 후 같은 재현을 다시 실행한다. 성능 변경은 동일 입력·환경의 전후 값과 결과 동등성을 증명하고 테스트 삭제로 통과시키지 않는다.
