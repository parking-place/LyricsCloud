...
- [ ] `LC-NF-1.0.5-P4-01` `AC-1.0.5-01`: [Verse: whisper] 뒤 가사 상황을 재현하여 “탐색은 Verse, 복사는 [Verse: whisper] 전체를 보존한다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.0.5-P4-02` `AC-1.0.5-02`: [Verse: a:b] 상황을 재현하여 “주 태그는 Verse이며 나머지 콜론과 내용이 손실되지 않는다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.0.5-P4-03` `AC-1.0.5-03`: 최종 복사 2999·3000·3001자 상황을 재현하여 “3001만 경고하며 저장과 복사는 항상 가능하다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.0.5-P4-04` `AC-1.0.5-04`: 부태그 조합 중 다른 탭 업데이트 상황을 재현하여 “한글 자모·cursor·undo가 깨지지 않는다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.0.5-P4-05` 직전 버전·다른 계정·오프라인·재접속·서버 재시작·지원 브라우저/기기 회귀를 수행하고 미실행/skip의 원인을 기록한다.
- [ ] `LC-NF-1.0.5-P4-06` 발견된 원인을 최소 수정 후 같은 재현을 다시 실행한다. 성능 변경은 동일 입력·환경의 전후 값과 결과 동등성을 증명하고 테스트 삭제로 통과시키지 않는다.
