...
- [ ] `LC-NF-1.0.5-P3-01` 좌측/모바일 송폼 탐색에 주 태그를 표시하고 필요 시 부태그를 보조 정보로 제공한다.
- [ ] `LC-NF-1.0.5-P3-02` 편집창에서 부태그를 낮은 강조로 표시하되 양 테마의 읽기 대비를 유지한다.
- [ ] `LC-NF-1.0.5-P3-03` 전체 복사에 3,000자 경고를 표시하고 성공/실패 복사 안내와 충돌하지 않게 한다.
- [ ] `LC-NF-1.0.5-P3-04` 기존 송폼 없음·중복 태그·잘못된 괄호에서도 편집·복사가 계속 가능하게 한다.
- [ ] `LC-NF-1.0.5-P3-05` 제안 `tests/e2e/new-feature-1.0.5.spec.ts`에 실제 기능 경로를 등록하고 정상·빈 상태·실패·권한 없음·로딩을 PC와 모바일에서 검사한다. native이면 플랫폼 runner와 UI 테스트로 대체/병행한다.
- [ ] `LC-NF-1.0.5-P3-06` 입력·선택·IME·undo·로컬 초안·서버 저장 상태의 연속성을 확인한다. 정상 UI만 보여주기 위해 오류/권한 검사를 제거하지 않는다.
