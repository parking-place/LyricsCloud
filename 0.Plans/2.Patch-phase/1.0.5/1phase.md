...
- [ ] `LC-NF-1.0.5-P1-01` [TAG:sub tag]에서 첫 콜론 앞 TAG만 탐색용 송폼 이름으로 사용한다.
- [ ] `LC-NF-1.0.5-P1-02` 부태그는 원문·검색·revision·전체 복사에서 유지하고 화면 강조만 줄인다.
- [ ] `LC-NF-1.0.5-P1-03` 가사 경고는 최종 Suno 전체 복사가 3,000자를 넘을 때만 표시한다.
- [ ] `LC-NF-1.0.5-P1-04` 관련 구현·테스트 경로를 읽고 위 요구/수용 사례를 정상·오류·권한·복구 입력표로 고정한다. 원인 미확정 항목은 가설과 증거 수준을 표시한다.
- [ ] `LC-NF-1.0.5-P1-05` 저장 형식·API·순서·copy·권한 중 변경되는 인터페이스와 호환/되돌림을 결정 문서에 기록하고 최초 소비 전에 승인한다.
- [ ] `LC-NF-1.0.5-P1-06` 각 변경 파일 담당자와 실제 최종 source SHA를 기록한다. 해당 패치 밖의 기능이나 아직 선택하지 않은 아이디어를 섞지 않는다.
