...
- 상태: **완료** (`complete`, 후보 `6dce2e363155ebd0da578a30c9c8185bf12564d5` 동일 SHA 개발 인수)
...
- [x] `LC-NF-1.0.5-P2-01` 구현 전 `1.0.5` 수용 사례의 실패 테스트를 작성한다. 실제 runner에 포함되는 `packages/editor/src/{songform,copy}.test.ts`에서 누락 projection/builder 3건의 실패를 확인했다.
- [x] `LC-NF-1.0.5-P2-02` 기존 송폼 문법을 포함하는 확장 파서를 만들고 raw 범위·주 이름·부태그 범위를 분리한다.
- [x] `LC-NF-1.0.5-P2-03` 반복 송폼의 고유 식별과 cursor/scroll 복원이 부태그 편집으로 잘못 이동하지 않게 했다.
- [x] `LC-NF-1.0.5-P2-04` 공통 copy serializer 뒤에서 길이를 재고 경고 때문에 원문을 정규화하지 않는다.
- [x] `LC-NF-1.0.5-P2-05` parser/index를 편집 transaction과 분리된 순수 projection으로 유지하고 suffix-only 증분 갱신과 section copy 원문 보존을 검증했다.
- [x] `LC-NF-1.0.5-P2-06` schema 변경 없음과 실제 PostgreSQL에서 migration 2회 반복 적용을 확인하고 동일 실패 테스트·전체 unit·check·build를 다시 통과했다.
...
- [x] 구현 Phase는 원격 PR #40과 동일 SHA 개발 인수를 갖췄다. 최종 필수 CI는 P5 후보에서 수행한다.
