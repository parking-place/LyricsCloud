...
- [ ] `LC-NF-1.0.13-P5-01` 제공자·언어별 범위·출처·캐시·오류·권리/개인정보 안내를 현재 docs에 반영한다.
- [ ] `LC-NF-1.0.13-P5-02` 관련 CI·동일 SHA 개발 HTTPS에서 세 언어 조회와 키보드/터치 smoke를 인수한다.
- [ ] `LC-NF-1.0.13-P5-03` 외부 제공 gate가 남으면 기능 전체 완료를 선언하지 않고 1.0.14 담당자에게 기존 편집 불변조건과 남은 결정을 넘긴다.
