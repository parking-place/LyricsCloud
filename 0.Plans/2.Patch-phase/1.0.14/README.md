...
상태: **P1~P5 완료 / 승인된 정식 릴리스 실행**. Noto Sans KR 2.004 Regular 자산·OFL·캐시, 선택 영속·세 편집기 적용, slow/blocked/offline·장문 입력·성능 회귀와 동일 SHA 개발 인수를 완료했다. 실제 Windows/Android/iOS의 1.0.14 폰트 전환은 미실행 위험으로 유지한다. 요구 `NF-REQ-044`를 [전체 추적표](../Requirements-Traceability.md)로 연결한다.
