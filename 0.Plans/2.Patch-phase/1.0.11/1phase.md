...
- [ ] `LC-NF-1.0.11-P1-01` 자동 metadata의 공식 접근 가능 범위와 이용 조건을 구현 시 확인한다. 비공개 곡 인증·스크래핑 우회는 금지한다.
- [ ] `LC-NF-1.0.11-P1-02` 수동 링크는 자동 조회 실패와 무관하게 사용 가능해야 한다. 실패를 성공으로 표시하지 않는다.
- [ ] `LC-NF-1.0.11-P1-03` 외부 조회는 SSRF·리다이렉트·DNS·크기/시간·이미지 추적 문제를 별도로 다룬다.
- [ ] `LC-NF-1.0.11-P1-04` 관련 구현·테스트 경로를 읽고 위 요구/수용 사례를 정상·오류·권한·복구 입력표로 고정한다. 원인 미확정 항목은 가설과 증거 수준을 표시한다.
- [ ] `LC-NF-1.0.11-P1-05` 저장 형식·API·순서·copy·권한 중 변경되는 인터페이스와 호환/되돌림을 결정 문서에 기록하고 최초 소비 전에 승인한다.
- [ ] `LC-NF-1.0.11-P1-06` 각 변경 파일 담당자와 실제 최종 source SHA를 기록한다. 해당 패치 밖의 기능이나 아직 선택하지 않은 아이디어를 섞지 않는다.
...
[1.0.11 P2](2phase.md)에 입력·실행 결과·호환 및 중단 조건을 넘긴다. 다음 단계의 승인이 없거나 선행 증거가 부족하면 자동 진행하지 않는다. 문서 작성으로 runtime version을 바꾸지 않는다.
