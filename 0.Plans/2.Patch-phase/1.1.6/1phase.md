...
- [1.1.5 P5](../1.1.5/5phase.md)의 산출물·검증/승인과 실제 최종 SHA 인수.
...
- [ ] `LC-NF-1.1.6-P1-01` 보이는 편집창 변화와 CRDT/CodeMirror 생명주기를 분리한다.
- [ ] `LC-NF-1.1.6-P1-02` 원문·selection·undo·scroll·IME·미전송 queue가 디자인 변경 전후 유지되어야 한다.
- [ ] `LC-NF-1.1.6-P1-03` beta/공유/탈퇴/복구의 보안 의미는 꾸미기 과정에서 축약하지 않는다.
- [ ] `LC-NF-1.1.6-P1-04` 관련 구현·테스트 경로를 읽고 위 요구/수용 사례를 정상·오류·권한·복구 입력표로 고정한다. 원인 미확정 항목은 가설과 증거 수준을 표시한다.
- [ ] `LC-NF-1.1.6-P1-05` 저장 형식·API·순서·copy·권한 중 변경되는 인터페이스와 호환/되돌림을 결정 문서에 기록하고 최초 소비 전에 승인한다.
- [ ] `LC-NF-1.1.6-P1-06` 각 변경 파일 담당자와 실제 최종 source SHA를 기록한다. 해당 패치 밖의 기능이나 아직 선택하지 않은 아이디어를 섞지 않는다.
