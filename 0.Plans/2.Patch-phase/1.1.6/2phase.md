...
- [1.1.6 P1](1phase.md)의 산출물·검증/승인과 실제 최종 SHA 인수.
...
- [ ] `LC-NF-1.1.6-P2-01` 구현 전 `1.1.6` 수용 사례의 실패 테스트를 작성한다. 제안 위치는 `tests/new-feature/1.1.6.contract.test.ts`이며 runner 포함 여부를 확인한 뒤 실패 이유를 기록한다.
- [ ] `LC-NF-1.1.6-P2-02` 편집기 wrapper·패널 상태·모바일 sheet를 승인된 구조로 이행한다.
- [ ] `LC-NF-1.1.6-P2-03` 이벤트 중복 등록·effect 정리·timer·portal stacking의 회귀를 먼저 고정한다.
- [ ] `LC-NF-1.1.6-P2-04` copy payload·송폼 파서·권한 검사·저장 완료 판단은 기존 계약을 재사용한다.
- [ ] `LC-NF-1.1.6-P2-05` 성능 전후 비교에서 장문 입력 지연과 메모리 회수를 확인하고 근거 있는 최적화만 적용한다.
- [ ] `LC-NF-1.1.6-P2-06` 추가 schema가 있으면 실제 테스트 DB의 빈 설치·이전 schema 업그레이드·권한·되돌림을 검사한다. 원인 수정 후 동일 실패 테스트와 기존 관련 회귀를 다시 실행한다.
