...
- 상태: **진행 중** (`in_progress`, P2 동일 SHA 개발 인수 완료·실패 우선 회귀 착수)
...
- [ ] `LC-NF-1.1.6-P3-01` 가사·라임·프롬프트 작성과 자료 패널의 양 테마·모바일 표시를 완성한다.
- [ ] `LC-NF-1.1.6-P3-02` signup·beta code 오류·공유 설정·권한 회수·탈퇴·초안 복구 상태를 일관되게 적용한다.
- [ ] `LC-NF-1.1.6-P3-03` 메뉴/시트/대화상자·가상 키보드의 겹침과 focus 복귀를 화면군 전체에서 확인한다.
- [ ] `LC-NF-1.1.6-P3-04` 실제 구현 스크린샷과 README/사용자 가이드를 합성 자료로 갱신한다.
- [ ] `LC-NF-1.1.6-P3-05` 제안 `tests/e2e/new-feature-1.1.6.spec.ts`에 실제 기능 경로를 등록하고 정상·빈 상태·실패·권한 없음·로딩을 PC와 모바일에서 검사한다. native이면 플랫폼 runner와 UI 테스트로 대체/병행한다.
- [ ] `LC-NF-1.1.6-P3-06` 입력·선택·IME·undo·로컬 초안·서버 저장 상태의 연속성을 확인한다. 정상 UI만 보여주기 위해 오류/권한 검사를 제거하지 않는다.
