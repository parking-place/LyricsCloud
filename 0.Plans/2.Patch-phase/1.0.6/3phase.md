...
- [ ] `LC-NF-1.0.6-P3-01` 우클릭 메뉴에 기본 Verse/Chorus/Bridge 등 승인 송폼 목록을 제공한다.
- [ ] `LC-NF-1.0.6-P3-02` 메뉴를 여닫아도 선택 범위·scroll·focus가 잃어버리지 않게 한다.
- [ ] `LC-NF-1.0.6-P3-03` 모바일 버튼과 키보드 명령으로 같은 목록/삽입 동작을 제공한다.
- [ ] `LC-NF-1.0.6-P3-04` Extend가 제외되는 복사 모드임을 설명하고 필요할 때 원문 보관본을 선택하게 한다.
- [ ] `LC-NF-1.0.6-P3-05` 제안 `tests/e2e/new-feature-1.0.6.spec.ts`에 실제 기능 경로를 등록하고 정상·빈 상태·실패·권한 없음·로딩을 PC와 모바일에서 검사한다. native이면 플랫폼 runner와 UI 테스트로 대체/병행한다.
- [ ] `LC-NF-1.0.6-P3-06` 입력·선택·IME·undo·로컬 초안·서버 저장 상태의 연속성을 확인한다. 정상 UI만 보여주기 위해 오류/권한 검사를 제거하지 않는다.
