# 1.0.6 Phase 5 — 문서·개발 인수·후속 연결

- 상태: **완료** (`complete`, 최종 후보·CI·동일 SHA 개발 인수)
- 단계 목적: Extend 작업 메모·송폼 삽입 메뉴의 문서·개발 인수·후속 연결을 완료하고 다음 단계에 검증 가능한 입력을 전달한다.
...
- [1.0.6 P4](4phase.md)의 산출물·검증/승인과 실제 최종 SHA 인수.
...
- [x] `LC-NF-1.0.6-P5-01` 1.0.6의 요구 범위와 각 수용 사례를 정확한 source SHA·변경 파일·migration·실행 명령·결과에 연결한다.
- [x] `LC-NF-1.0.6-P5-02` 사용자 가이드·README·지원·보안·자가호스팅·현재 계획/결정 문서를 Extend 작업 메모·송폼 삽입 메뉴의 실제 동작에 맞춰 갱신하고 과거 검증 기록은 보존한다.
- [x] `LC-NF-1.0.6-P5-03` 원격 CI와 허용된 개발 산출물·동일 SHA의 개발 환경을 확인한다. 문서-only 결과면 문서 승인만 기록하고 앱 배포 성공을 주장하지 않는다.
- [x] `LC-NF-1.0.6-P5-04` 확인된 P0/P1, 원문 유실, 인증 우회, 무음 저장 실패가 없음을 실제 범위에서 판정한다. 신규 범위의 필수 안전성을 다음 안정화 패치로 미루지 않는다.
- [x] `LC-NF-1.0.6-P5-05` 다섯 Phase 후보 인수 이후에만 별도 release go/no-go를 요청할 수 있다. main 병합·정식 별칭·app.example.test 배포는 현재 승인 범위를 확인하고 실행한다.
- [x] `LC-NF-1.0.6-P5-06` 후속 패치에 계약·미해결/보류·실기기/운영 증거·중단/복구 조건을 인계한다. 같은 제품 계열을 유지하고 빈 릴리스·자동 minor 승격을 만들지 않는다.
...
- 1.0.6 P5의 검토 가능한 변경/계약과 수용 사례 증거.
- 변경 파일·추가 migration·실행 명령/환경·결과·미실행/잔여 위험의 인수 기록.
- 후속 소비자가 유지할 API·데이터·copy·권한·UI 상태 계약.

## 다음 Phase 인계

[1.0.7 P1](../1.0.7/1phase.md)에 입력·실행 결과·호환 및 중단 조건을 넘긴다. 다음 단계의 승인이 없거나 선행 증거가 부족하면 자동 진행하지 않는다. 문서 작성으로 runtime version을 바꾸지 않는다.

## 완료 기록

P5 후보 `a37a41dde72974fb37e025788ace768f9d5b6b04`는 GitHub Actions run `34511266182`에서 migration 반복·unit/integration·production build/image·upgrade/rollback·취약점/secret·전체 auth/ownership E2E·지원 브라우저 릴리즈 행렬을 통과했고 web·collaboration·worker·migrate 개발 image 게시·서명도 통과했다. 같은 SHA를 개발 서버에 배포해 공개 live/ready의 `1.0.6`·`dev`·`p5`, schema `1000_prompt_modes.sql`과 네 서비스 health를 확인했다. 공개 revision·ZIP JSON/TXT의 exact Extend 원문과 collaboration 실제 재시작 전후 동일 document key·원문 보존이 PASS했고 합성 자료를 제거했다. 신규 P0/P1·원문 유실·인증 우회·교차 owner 노출·무음 저장 실패는 0건이다. 실제 물리 기기 미실행과 `OPS-100-001` 예외를 유지하며 사용자가 승인한 정식 릴리스로 이동한다.
