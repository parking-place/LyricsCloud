...
- [ ] `LC-NF-1.0.6-P4-01` `AC-1.0.6-01`: [Extend: 3:00:24] 뒤 두 줄 가사 전체 복사 상황을 재현하여 “표식만 제외되고 가사 두 줄은 남는다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.0.6-P4-02` `AC-1.0.6-02`: 원문 export·revision 복원 상황을 재현하여 “Extend 표식과 시간 문자열이 그대로 남는다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.0.6-P4-03` `AC-1.0.6-03`: 우클릭 후 다른 탭이 앞부분 수정 상황을 재현하여 “안전한 상대 위치에 한 번만 삽입되거나 보존 안내가 나온다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.0.6-P4-04` `AC-1.0.6-04`: IME 조합 중 메뉴·Escape·undo 상황을 재현하여 “조합 원문이 보존되고 한 번 undo로 삽입만 취소된다.”를 검증한다. 자동/실제 DB/브라우저/실기기 증거를 구분한다.
- [ ] `LC-NF-1.0.6-P4-05` 직전 버전·다른 계정·오프라인·재접속·서버 재시작·지원 브라우저/기기 회귀를 수행하고 미실행/skip의 원인을 기록한다.
- [ ] `LC-NF-1.0.6-P4-06` 발견된 원인을 최소 수정 후 같은 재현을 다시 실행한다. 성능 변경은 동일 입력·환경의 전후 값과 결과 동등성을 증명하고 테스트 삭제로 통과시키지 않는다.
