# 1.0.6 Phase 4 — 실패·권한·복구 회귀

- 상태: **완료** (`complete`, 실패·권한·복구 교차 인수)
- 단계 목적: Extend 작업 메모·송폼 삽입 메뉴의 실패·권한·복구 회귀을 완료하고 다음 단계에 검증 가능한 입력을 전달한다.
...
- [1.0.6 P3](3phase.md)의 산출물·검증/승인과 실제 최종 SHA 인수.
...
- [x] `LC-NF-1.0.6-P4-01` `[Extend: 3:00:24]` 뒤 두 줄에서 Suno 전체 copy가 표식 한 줄만 제외하고 두 가사 줄을 보존함을 단위·브라우저·실제 PostgreSQL에서 확인했다.
- [x] `LC-NF-1.0.6-P4-02` 원문 revision 복원과 계정 ZIP의 JSON·사람용 TXT에서 Extend 표식과 시간 문자열이 동일함을 로컬과 공개 개발 환경에서 확인했다.
- [x] `LC-NF-1.0.6-P4-03` 메뉴가 캡처한 CRDT 상대 caret 앞에 다른 탭이 접두어를 삽입해도 안전한 위치에 한 번만 송폼을 넣고 한 번의 undo로 제거함을 확인했다.
- [x] `LC-NF-1.0.6-P4-04` Chromium desktop 합성 조합 중 메뉴 차단, 조합 종료 뒤 Escape·삽입·단일 undo와 원문 보존을 확인했다. 실제 OS IME는 새로 실행하지 않았다.
- [x] `LC-NF-1.0.6-P4-05` 실제 PostgreSQL, 다른 계정, offline/재접속, 전체 Chromium PC/mobile, Firefox/WebKit 기능 행렬과 동일 SHA 개발 collaboration 재시작을 수행했다.
- [x] `LC-NF-1.0.6-P4-06` 구 metadata 기대값과 dark disabled danger 대비를 최소 수정하고 영향 시험과 전체 회귀를 다시 통과했다. 테스트 assertion이나 제품 한계를 완화하지 않았다.
...
- [x] 작업 ID마다 코드·실행/검토 증거와 후보 `0d19330f9c8ba800e89b275a22fedf1b9e955693`을 연결했다.
- [x] 현재 패치의 원문·권한·복구·오류 처리를 정상 동작과 함께 검증했다.
- [x] 실제 물리 기기 미실행과 조건부 skip 원인을 숨김없이 기록했다.
- [x] 현재 상태/담당/변경 파일·관련 문서가 실제 수행 내용과 일치한다.
- [x] 로컬 전체 회귀와 동일 SHA 개발 서버·공개 revision/export·collaboration 재시작 인수를 완료했다. 최종 원격 CI는 P5 후보에서 수행한다.
- [x] main·Release·릴리스 서버 변경은 수행하지 않았다.

## 산출물

- 1.0.6 P4의 검토 가능한 변경/계약과 수용 사례 증거.
- 변경 파일·추가 migration·실행 명령/환경·결과·미실행/잔여 위험의 인수 기록.
- 후속 소비자가 유지할 API·데이터·copy·권한·UI 상태 계약.

## 다음 Phase 인계

[1.0.6 P5](5phase.md)에 입력·실행 결과·호환 및 중단 조건을 넘긴다. 다음 단계의 승인이 없거나 선행 증거가 부족하면 자동 진행하지 않는다. 문서 작성으로 runtime version을 바꾸지 않는다.
