...
- [ ] `LC-NF-1.0.6-P1-01` 정확한 Extend 표식의 토큰/행 제거 범위를 정한다. 뒤의 가사 섹션까지 삭제하지 않는다.
- [ ] `LC-NF-1.0.6-P1-02` [Extend: 3:00:24] 시간 문자열은 사용자 메모로 보존하고 임의 시간 단위 변환을 하지 않는다.
- [ ] `LC-NF-1.0.6-P1-03` 우클릭은 보조 수단이다. 키보드 호출과 모바일 보이는 삽입 버튼을 함께 둔다.
- [ ] `LC-NF-1.0.6-P1-04` 관련 구현·테스트 경로를 읽고 위 요구/수용 사례를 정상·오류·권한·복구 입력표로 고정한다. 원인 미확정 항목은 가설과 증거 수준을 표시한다.
- [ ] `LC-NF-1.0.6-P1-05` 저장 형식·API·순서·copy·권한 중 변경되는 인터페이스와 호환/되돌림을 결정 문서에 기록하고 최초 소비 전에 승인한다.
- [ ] `LC-NF-1.0.6-P1-06` 각 변경 파일 담당자와 실제 최종 source SHA를 기록한다. 해당 패치 밖의 기능이나 아직 선택하지 않은 아이디어를 섞지 않는다.
