...
- 상태: **진행 중** (`in_progress`, PC·모바일 selected-writer 사용자 흐름 구현)
...
- [ ] `LC-NF-1.1.2-P3-01` owner가 지정 writer를 선택하고 실제 reader 집합과 불일치할 때 저장을 막는다.
- [ ] `LC-NF-1.1.2-P3-02` writer 편집기에 권한·저장·연결·회수 상태를 표시한다.
- [ ] `LC-NF-1.1.2-P3-03` 권한 회수 시 입력을 더 서버에 보내지 않고 본인이 작성한 미전송 부분의 보관 방법을 제공한다.
- [ ] `LC-NF-1.1.2-P3-04` 두 사용자·두 기기에서 selection·한글 IME·undo·복원 UI가 서로의 작업을 무음 삭제하지 않게 한다.
- [ ] `LC-NF-1.1.2-P3-05` 제안 `tests/e2e/new-feature-1.1.2.spec.ts`에 실제 기능 경로를 등록하고 정상·빈 상태·실패·권한 없음·로딩을 PC와 모바일에서 검사한다. native이면 플랫폼 runner와 UI 테스트로 대체/병행한다.
- [ ] `LC-NF-1.1.2-P3-06` 입력·선택·IME·undo·로컬 초안·서버 저장 상태의 연속성을 확인한다. 정상 UI만 보여주기 위해 오류/권한 검사를 제거하지 않는다.

- [ ] `LC-NF-1.1.2-P3-07` 현재 보는 사람과 누가 어느 구간을 작업하는지 표시하고 늦은 위치·disconnect·idle·중복 탭·자기 caret 충돌 상태를 처리한다.
