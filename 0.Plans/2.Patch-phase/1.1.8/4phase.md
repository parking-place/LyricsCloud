...
2026-09-16 사용자 인계: 별도 런타임 설치 없이 실행 가능한 Windows 빌드가 준비되면 사용자 PC에서 실행해 보겠다고 했고, 그 전 실기기 검증은 보류해 달라고 요청했다. 이 발언은 Windows 앱 launch/OAuth/DPAPI/clipboard/IME/AT/DPI·서명 설치/업데이트 PASS가 아니다. `P4-06`은 기존 framework-dependent 개발 artifact와 별개로 .NET/Windows App SDK runtime을 담는 unpackaged x64 시험 폴더를 CI에 추가했다. workflow YAML parse와 diff check는 PASS; 실제 Windows publish artifact·깨끗한 PC 실행은 원격 CI/사용자 검증 전까지 **미확인**이다. P4 `review`, P5·main/Release·릴리스 서버 보류를 유지한다.
