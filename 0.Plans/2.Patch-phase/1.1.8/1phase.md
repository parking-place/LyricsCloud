...
- 상태: **검토** (`review`, 기술 비교·reference spike 완료 / 사용자 승인·최종 source SHA 대기)
...
- [ ] `LC-NF-1.1.8-P1-05` 저장 형식·API·순서·copy·권한 중 변경되는 인터페이스와 호환/되돌림을 결정 문서에 기록하고 최초 소비 전에 승인한다.
- [ ] `LC-NF-1.1.8-P1-06` 각 변경 파일 담당자와 실제 최종 source SHA를 기록한다. 해당 패치 밖의 기능이나 아직 선택하지 않은 아이디어를 섞지 않는다.
...
권고 선택은 **WinUI 3/.NET native UI, 1.1.8 read/copy-only, WebView2 미포함, system browser + first-party loopback PKCE, account별 DPAPI cache**다. `LC-NF-1.1.8-P1-05`의 최초 소비 승인과 `P1-06`의 승인된 최종 source SHA는 사용자 응답 뒤 기록한다. 그 전에는 P1을 완료 처리하거나 P2로 이동하지 않는다.
