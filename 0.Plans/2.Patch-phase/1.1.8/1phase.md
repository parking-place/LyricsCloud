...
- [ ] `LC-NF-1.1.8-P1-01` 필수 요청은 네이티브 개발안이며 실제 앱 구현은 사용자 승인 이후만 진행한다. 승인 전 P2 진입을 막는다.
- [ ] `LC-NF-1.1.8-P1-02` WinUI/Avalonia/부분 WebView 등 후보를 비교하고 한국어 편집·Yjs 호환의 최소 실험을 수행한다.
- [ ] `LC-NF-1.1.8-P1-03` 첫 앱 범위는 읽기·복사이며 편집 완료 제품으로 설명하지 않는다. 문서/실험만으로 release tag를 발행하지 않는다.
- [ ] `LC-NF-1.1.8-P1-04` 관련 구현·테스트 경로를 읽고 위 요구/수용 사례를 정상·오류·권한·복구 입력표로 고정한다. 원인 미확정 항목은 가설과 증거 수준을 표시한다.
...
- [ ] `LC-NF-1.1.8-P1-07` 웹의 다사용자 협업/presence·사전 제공 경로·무료 폰트/라이선스 계약을 플랫폼 개발안에 포함하고 OS별 미지원/대체 승인 gate를 명시한다.
