...
사용자가 1.0.1 전체 Phase 완료 후 릴리스 서버까지 배포하도록 명시 승인해 대안 2를 선택했다. 각 Phase는 전용 `phase/` 브랜치·검증·개발 서버 인수를 거치고, P10 최종 후보만 release PR로 `main`에 병합한다. 정식 이미지와 `Release`, `latest`, `Release-latest` 별칭은 최종 main SHA 검증 후에만 이동하며 기존 v1.0.0 이력은 변경하지 않는다.
