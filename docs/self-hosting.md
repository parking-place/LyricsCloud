# LyricsCloud 1.0.2 정식·1.0.3 후보 셀프호스팅
...
현재 정식 설치 기준은 불변 `v1.0.2` tag다. 1.0.3 출시 전 후보 검토는 승인된 전체 40자리 commit SHA만 사용한다. moving alias나 branch 이름을 운영 checkout 기준으로 쓰지 않는다.
...
정식 환경 이름·형식의 기준은 봉인된 [1.0.2 environment schema](../config/environment-schema.1.0.2.json)다. 1.0.3 후보 계약은 [1.0.3 environment schema](../config/environment-schema.1.0.3.json)를 사용하고 `APP_VERSION=1.0.3`, `APP_CHANNEL=dev`, `APP_PHASE=p5`, `BUILD_ID=<후보 SHA>`로만 검증하며 release 설정·별칭으로 승격하지 않는다. Google 설정은 아래 OAuth 절차를 먼저 마친다. 릴리스 적용 전에는 해당 tag에 포함된 schema로 web·collaboration·worker·migrate·admin 설정을 각각 검사한다.
...
두 endpoint는 200이어야 하고 1.0.2 ready 응답의 `version`, `build.id`, `channel`, `phase`, `database.latestMigration`은 각각 `1.0.2`, checkout SHA, `release`, `null`, `0901_beta_signup.sql`이어야 한다. 1.0.3 개발 후보는 `channel=dev`, 해당 `phase`, `1000_prompt_modes.sql`을 반환한다. `/api/health/metrics` 같은 공개 metrics 경로는 제공하지 않는다. 보호 API는 비로그인 요청을 거부해야 한다.
