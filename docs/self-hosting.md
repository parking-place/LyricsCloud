# LyricsCloud 1.0.12 정식·1.0.14 후보 셀프호스팅
...
정식 환경 이름·형식의 기준은 봉인된 [1.0.12 environment schema](../config/environment-schema.1.0.12.json)다. 1.0.14 후보는 [1.0.14 environment schema](../config/environment-schema.1.0.14.json)에서 `APP_VERSION=1.0.14`, `APP_CHANNEL=dev`, `APP_PHASE=p5`, `BUILD_ID=<후보 SHA>`로 검증한다. Google 설정은 아래 OAuth 절차를 먼저 마친다. 릴리스 적용 전에는 해당 tag에 포함된 schema로 web·collaboration·worker·migrate·admin 설정을 각각 검사한다.
...
두 endpoint는 200이어야 하고 1.0.12 ready 응답의 `version`, `build.id`, `channel`, `phase`, `database.latestMigration`은 각각 `1.0.12`, checkout SHA, `release`, `null`, `1003_song_suno_workspaces.sql`이어야 한다. 1.0.14 개발 후보는 schema `1004_web_font_selection.sql`, `channel=dev`와 해당 `phase`를 반환한다. `/api/health/metrics` 같은 공개 metrics 경로는 제공하지 않는다. 보호 API는 비로그인 요청을 거부해야 한다. 1.0.14 reverse proxy는 hash OTF의 content type과 1년 immutable cache header를 보존한다.
