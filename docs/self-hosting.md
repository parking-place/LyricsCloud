# LyricsCloud 1.0.6 정식·1.0.7 후보 셀프호스팅
...
현재 정식 설치 기준은 불변 `v1.0.6` tag다. 1.0.7 후보 검토는 승인된 전체 40자리 commit SHA만 사용한다. moving alias나 branch 이름을 운영 checkout 기준으로 쓰지 않는다.
...
두 endpoint는 200이어야 하고 1.0.6 ready 응답의 `version`, `build.id`, `channel`, `phase`, `database.latestMigration`은 각각 `1.0.6`, checkout SHA, `release`, `null`, `1000_prompt_modes.sql`이어야 한다. 1.0.7 개발 후보는 schema `1001_library_view_settings.sql`, `channel=dev`와 해당 `phase`를 반환한다. `/api/health/metrics` 같은 공개 metrics 경로는 제공하지 않는다. 보호 API는 비로그인 요청을 거부해야 한다.
