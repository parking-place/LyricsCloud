# LyricsCloud 1.1.7a 정식·1.2.0 개발 후보 셀프호스팅
...
현재 정식 설치 기준은 불변 `v1.1.7a` tag다. 1.2.0 개발 후보 검토는 승인된 전체 40자리 commit SHA만 사용한다. moving alias나 branch 이름을 운영 checkout 기준으로 쓰지 않는다.

```bash
git clone https://github.com/parking-place/LyricsCloud.git
cd LyricsCloud
APPROVED_REF=v1.1.7a
git checkout --detach "$APPROVED_REF"
cp .env.example .env
cp .test_users.example .test_users
```

`.env`의 모든 `CHANGE_ME`를 바꾼다. 정식 설치는 `NODE_ENV=production`, `APP_VERSION=1.1.7a`, `BUILD_ID=fc2463cdb47d9fd7d0042779f602c6ddb7d734cf`, `APP_CHANNEL=release`, `APP_ORIGIN=https://<호스트>`로 설정하고 `APP_PHASE`를 비운다. 정식판은 `LC_UI_VARIANT=b1`, 시각 복귀에만 `classic`을 사용한다. 별도 개발 환경의 1.2.0 후보는 `APP_CHANNEL=dev`, `APP_PHASE=p5`, `LC_UI_VARIANT=chroma`를 선택할 수 있으며 운영 DB/secret을 재사용하지 않는다. `DATABASE_URL`의 사용자·비밀번호·DB명은 같은 파일의 PostgreSQL 값과 일치해야 한다. `SESSION_SECRET`은 32바이트 이상의 무작위 값이어야 한다. 실제 값은 채팅·Issue·Git·image build argument에 넣지 않는다.
...
정식 환경 이름·형식의 기준은 봉인된 [1.1.7a environment schema](../config/environment-schema.1.1.7a.json)다. 1.2.0 개발 후보는 별도 환경에서 [1.2.0 environment schema](../config/environment-schema.1.2.0.json)로 `APP_VERSION=1.2.0`, `APP_CHANNEL=dev`, `APP_PHASE=p5`, `BUILD_ID=<후보 SHA>`와 선택한 `LC_UI_VARIANT`를 검증한다. Google 설정은 아래 OAuth 절차를 먼저 마친다. 릴리스 적용 전에는 해당 tag에 포함된 schema로 web·collaboration·worker·migrate·admin 설정을 각각 검사한다.
...
두 endpoint는 200이어야 하고 현재 정식 1.1.7a ready 응답의 `version`, `build.id`, `channel`, `phase`, `database.latestMigration`은 각각 `1.1.7a`, `fc2463cdb47d9fd7d0042779f602c6ddb7d734cf`, `release`, `null`, `1151_profile_customization.sql`이어야 한다. 별도 1.2.0 개발 후보는 `1.2.0`, 후보 SHA, `dev`, `p5`, `1152_prompt_dictionary_cascade.sql`이어야 한다. `/api/health/metrics` 같은 공개 metrics 경로는 제공하지 않는다. 보호 API는 비로그인 요청을 거부해야 한다. reverse proxy는 hash OTF cache header와 `/collaboration/*`의 HTTP·WebSocket upgrade, origin·cookie 전달을 보존한다.
