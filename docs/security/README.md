...
1.1.7의 `returnTo`는 same-origin 내부 pathname/query만 허용하고 없거나 유효하지 않으면 기존 자료 목록으로 돌아간다. 목록 filter·sort·view query, focus tooltip과 접근성 media-query fallback은 client navigation·presentation만 바꾸며 API·DB·권한·본문을 새 로그나 URL에 넣지 않는다. 외부 URL redirect, query의 창작물 원문, 다른 owner route, ACK 전 저장 성공이 관찰되면 배포를 중단하고 `LC_UI_VARIANT=classic` 또는 1.1.6 exact image로 application-first rollback한다.
