...
사용자가 1.0.1의 모든 Phase 실행과 전체 완료 뒤 릴리스 서버 배포를 명시 승인했다. 이에 따라 대안 2인 `hash seed + DB grant/receipt`를 선택한다. P1 계약의 초기 운영값은 활성 코드 100개, 기본 만료 24시간, principal별 실패 5회/24시간, 환경별 전체 실패 1,000회/24시간으로 시작하며 실제 운영 증거에 따라 별도 승인 변경한다. 평문 허용 fallback, 웹 런타임의 코드 복호화, 정지·탈퇴 계정의 재활성화는 허용하지 않는다.
