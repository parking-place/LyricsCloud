...
- `.env`, HMAC JSONL `.test_users`, `.private/keys/auth_allowlist_hmac_keyring`은 mode `600`으로 만들고 Git·Docker build context에 넣지 않는다. 평문 bootstrap을 아직 전환하지 않았다면 [P3 운영 인수](./1.0.1-phase3-hmac-allowlist.md)를 먼저 수행한다.
- Docker Compose의 local file secret은 host permission을 그대로 bind하므로, 배포 script가 allowlist와 keyring을 `.private/runtime/`에 UID `65532`·mode `400`으로 각각 원자 복사한다. 원본 mode `600`은 바꾸지 않으며 runtime copy도 Git·Docker build context에서 제외한다.
...
- `.env`, HMAC `.test_users` 또는 HMAC keyring이 비어 있는 경우
- Distroless UID `65532` 전용 allowlist·keyring runtime copy의 소유권·mode를 만들거나 검증하지 못한 경우
