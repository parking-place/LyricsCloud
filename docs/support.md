# LyricsCloud 1.0.10 정식·1.0.12 후보 지원 정책
...
1.0.12 후보는 곡·라임·프롬프트 목록의 위치 계산을 결과 동등 O(n) 공통 경로로 바꾸고, 기존 작성·copy·정렬·Suno 수동 링크를 desktop/mobile 수직 흐름과 서비스 재시작으로 재검증한다. 실제 PostgreSQL 330건, Actions `34632758817`·`34632766602`, 동일 SHA 개발 owner 404·PWA·재시작 지속성을 통과했다. 자동 metadata·공동 편집·실제 물리 기기 인수는 포함하지 않으며 P5 전체 CI 전에는 정식 지원 버전으로 보지 않는다.
