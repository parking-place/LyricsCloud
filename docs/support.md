# LyricsCloud 지원 정책 — 1.1.7a 정식·1.1.7b 개발 후보

현재 정식 운영 버전은 `1.1.7a`다. `1.1.7b`는 웹 안정화 개발 후보로 정식 tag·운영 배포 승인이 없다. b에서 수정한 입력·공유·목록·백업 잠금 동작과 `1152` DB 호환은 [후보 노트](./releases/1.1.7b.md)를 따르되, 기존 템플릿 복제 후 목록/오류 안내(UI-04/05)와 오류 읽기 위치(UI-07)는 미해결 P2다. b의 실제 Windows/iOS/Android 기기·OS IME·AT는 사용자 지시로 후속 보류/미실행이며 자동 브라우저 결과를 새 실기기 PASS로 간주하지 않는다. 정식 지원·발행은 별도 승인/검증 뒤 갱신한다.
