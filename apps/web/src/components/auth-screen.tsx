...
export function AuthScreen({ errorCode, requestId, flow }: { errorCode?: string; requestId?: string; flow?: string }) {
  ...
  return <main className="auth-layout">
    ...
    <section className="auth-panel" aria-labelledby="login-title"><div className="auth-card"><span className="brand-mark card-mark" aria-hidden="true">L</span><h2 id="login-title">다시 작업을 시작해볼까요?</h2><p className="auth-description">로그인하면 PC와 모바일 어디서든 같은 곡, 가사, 라임 노트와 프롬프트를 이어서 사용할 수 있습니다.</p>{error ? <div className="auth-error" role="alert" tabIndex={-1}><strong>{error.title}</strong><span>{flow === "signup" && errorCode === "AUTH_CANCELLED" ? "코드는 사용되지 않았습니다. 가입을 다시 시작할 수 있습니다." : error.message}</span>{requestId ? <small>문의 코드 {requestId}</small> : null}</div> : null}<div className="mobile-sync-copy"><strong>어느 기기에서든 이어서</strong><span>마지막으로 쓰던 가사부터 바로 시작할 수 있습니다.</span></div><LoginButton /><BetaSignupForm /><p className="auth-foot">계속하면 <Link href="/terms">이용 안내</Link>와 <Link href="/privacy">개인정보 안내</Link>를 확인하고 동의한 것으로 봅니다.</p><p className="policy-version">정책 버전 2026-09-04</p></div></section>
  </main>;
}
