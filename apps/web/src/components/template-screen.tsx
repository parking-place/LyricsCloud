...
export function TemplateScreen({ initialQuery }: { initialQuery: TemplateListInput }) {
  ...
  const formatDrafts = useRef<Record<PromptMode, string>>({ tags: "", sentence: "" });
  ...
  function confirmDiscardDraft(): boolean {
    return mode === "view" || window.confirm("저장하지 않은 템플릿 작성 내용을 버릴까요?");
  }
  function beginCreate() {
    if (!confirmDiscardDraft()) return;
    formatDrafts.current = { tags: "", sentence: "" }; setTitle(""); setContent(""); setPromptMode("tags"); setMode("create"); setNotice("");
  }
  function beginEdit() {
    if (!selected || selected.source === "default") return;
    formatDrafts.current = { tags: selected.tokens.map((token) => token.displayValue).join(", "), sentence: selected.promptText ?? "" };
  ...
  function changePromptMode(next: PromptMode) {
    if (locked || next === promptMode) return;
    formatDrafts.current[promptMode] = content;
    setContent(formatDrafts.current[next]);
    setPromptMode(next);
  }
  ...
  return <section className="templates-page" aria-labelledby="templates-title" data-pending-input={mode !== "view"}>
    ...
    <div className="template-tabs" role="group" aria-label="템플릿 유형">{(["lyrics", "prompt"] as const).map((type) => <button disabled={locked} key={type} className={query.type === type ? "active" : ""} aria-pressed={query.type === type} onClick={() => { if (!confirmDiscardDraft()) return; setMode("view"); setQuery((value) => ({ ...value, type })); }}>{type === "lyrics" ? "가사 구조" : "프롬프트"}</button>)}</div>
    <div className="template-filters"><label>출처<select disabled={locked || mode !== "view"} value={query.source} onChange={(event) => setQuery((value) => ({ ...value, source: event.target.value as TemplateListInput["source"] }))}><option value="all">기본 + 내 템플릿</option><option value="default">기본 템플릿</option><option value="user">내 템플릿</option></select></label><label>정렬<select disabled={locked || mode !== "view"} value={query.sort} onChange={(event) => setQuery((value) => ({ ...value, sort: event.target.value as TemplateListInput["sort"] }))}><option value="favorite_first">즐겨찾기 우선</option><option value="recent_used">최근 사용</option><option value="updated_desc">최근 수정</option><option value="title_asc">이름순</option></select></label></div>
    {notice ? <p className="template-notice" role="status">{notice}</p> : null}
    <div className="templates-layout">
      <aside className="template-list" aria-label="템플릿 목록">{loading ? <p>템플릿을 불러오는 중…</p> : items.length ? items.map((item) => <button disabled={locked} key={item.id} className={selected?.id === item.id ? "selected" : ""} onClick={() => { if (!confirmDiscardDraft()) return; setSelectedId(item.id); setMode("view"); }}><span><strong>{item.title}</strong><small>{item.source === "default" ? "기본 · 읽기 전용" : "내 템플릿"}</small></span><span aria-label={item.isFavorite ? "즐겨찾기" : undefined}>{item.isFavorite ? "★" : ""}</span></button>) : <div className="template-empty"><strong>내 템플릿이 없습니다</strong><p>새 템플릿을 만들거나 기본 템플릿을 복제해 시작하세요.</p><button type="button" disabled={locked} onClick={beginCreate}>첫 템플릿 만들기</button></div>}</aside>
      <article className="template-preview">
        {mode !== "view" ? <form onSubmit={save}><p className="eyebrow">{mode === "create" ? "New template" : "Edit template"}</p><label>템플릿 제목<input autoFocus disabled={locked} value={title} maxLength={200} onChange={(event) => setTitle(event.target.value)} /></label>{(mode === "edit" ? selected?.type : query.type) === "prompt" ? <fieldset className="template-prompt-mode" disabled={locked}><legend>프롬프트 형식</legend><label><input type="radio" name="template-prompt-mode" checked={promptMode === "tags"} onChange={() => changePromptMode("tags")} />태그형</label><label><input type="radio" name="template-prompt-mode" checked={promptMode === "sentence"} onChange={() => changePromptMode("sentence")} />문장형</label></fieldset> : null}<label>{(mode === "edit" ? selected?.type : query.type) === "lyrics" ? "가사 구조 원문" : promptMode === "tags" ? "쉼표로 구분한 프롬프트 태그" : "프롬프트 문장 원문"}<textarea disabled={locked} value={content} rows={14} onChange={(event) => setContent(event.target.value)} /></label><footer><button type="button" className="secondary-button" disabled={locked} onClick={() => { if (confirmDiscardDraft()) setMode("view"); }}>취소</button><button className="primary-link" type="submit" disabled={saving}>저장</button></footer></form>
          : selected ? <><header><div><span className="template-source">{selected.source === "default" ? "기본 템플릿" : "내 템플릿"}{selected.type === "prompt" ? ` · ${selected.promptMode === "tags" ? "태그형" : "문장형"}` : ""}</span><h2>{selected.title}</h2><small>{selected.lastUsedAt ? `최근 사용 ${new Date(selected.lastUsedAt).toLocaleDateString("ko-KR", { timeZone: "Asia/Seoul" })}` : "아직 사용하지 않음"}</small></div><button type="button" className="template-favorite" aria-label={selected.isFavorite ? "즐겨찾기 해제" : "즐겨찾기 추가"} aria-pressed={selected.isFavorite} onClick={() => void favorite()}>{selected.isFavorite ? "★ 즐겨찾기" : "☆ 즐겨찾기"}</button></header><div className="template-content" aria-label="템플릿 내용 미리보기">{selected.type === "lyrics" ? <pre>{selected.lyricBody}</pre> : selected.promptMode === "sentence" ? <pre>{selected.promptText}</pre> : <ol>{selected.tokens.map((token, index) => <li key={`${token.normalizedValue}-${index}`}>{token.displayValue}</li>)}</ol>}</div><footer className="template-actions"><a className="primary-link" href={selected.type === "lyrics" ? `/lyrics/new?template=${selected.id}` : `/prompts/new?template=${selected.id}`}>이 템플릿으로 시작</a><button className="secondary-button" type="button" onClick={() => void duplicate()}>복제</button>{selected.source === "user" ? <><button className="secondary-button" type="button" onClick={beginEdit}>수정</button><button className="danger-button" type="button" onClick={() => void remove()}>삭제</button></> : null}</footer></> : <div className="template-empty"><strong>미리 볼 템플릿을 선택하세요</strong></div>}
      </article>
    </div>
  </section>;
}
