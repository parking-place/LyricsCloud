...
export function TemplateScreen({ initialQuery }: { initialQuery: TemplateListInput }) {
  ...
  async function save(event: React.FormEvent) {
    ...
    try {
      ...
      const response = mode === "create" ? await fetch("/api/templates", { method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ requestId: crypto.randomUUID(), type, title, ...payload }) })
        : await fetch(`/api/templates/${selected!.id}`, { method: "PUT", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ rowVersion: selected!.rowVersion, title, ...payload }) });
    ...
    } catch { setNotice("템플릿을 저장하지 못했습니다. 제목·내용 길이와 중복 토큰을 확인해 주세요."); }
  }
  ...
  return <section className="templates-page" aria-labelledby="templates-title">
    <header className="templates-heading"><div><p className="eyebrow">Reusable starting points · 0.9.0</p><h1 id="templates-title" tabIndex={-1} data-login-focus>템플릿</h1><p>반복하는 가사 구조와 프롬프트 조합을 원본과 분리해 새 작업에 적용하세요.</p></div><button className="primary-link" type="button" onClick={beginCreate}>＋ 새 템플릿</button></header>
    <div className="template-tabs" role="group" aria-label="템플릿 유형">{(["lyrics", "prompt"] as const).map((type) => <button key={type} className={query.type === type ? "active" : ""} aria-pressed={query.type === type} onClick={() => { setMode("view"); setQuery((value) => ({ ...value, type })); }}>{type === "lyrics" ? "가사 구조" : "프롬프트"}</button>)}</div>
    <div className="template-filters"><label>출처<select value={query.source} onChange={(event) => setQuery((value) => ({ ...value, source: event.target.value as TemplateListInput["source"] }))}><option value="all">기본 + 내 템플릿</option><option value="default">기본 템플릿</option><option value="user">내 템플릿</option></select></label><label>정렬<select value={query.sort} onChange={(event) => setQuery((value) => ({ ...value, sort: event.target.value as TemplateListInput["sort"] }))}><option value="favorite_first">즐겨찾기 우선</option><option value="recent_used">최근 사용</option><option value="updated_desc">최근 수정</option><option value="title_asc">이름순</option></select></label></div>
    {notice ? <p className="template-notice" role="status">{notice}</p> : null}
    <div className="templates-layout">
      <aside className="template-list" aria-label="템플릿 목록">{loading ? <p>템플릿을 불러오는 중…</p> : items.length ? items.map((item) => <button key={item.id} className={selected?.id === item.id ? "selected" : ""} onClick={() => { setSelectedId(item.id); setMode("view"); }}><span><strong>{item.title}</strong><small>{item.source === "default" ? "기본 · 읽기 전용" : "내 템플릿"}</small></span><span aria-label={item.isFavorite ? "즐겨찾기" : undefined}>{item.isFavorite ? "★" : ""}</span></button>) : <div className="template-empty"><strong>내 템플릿이 없습니다</strong><p>새 템플릿을 만들거나 기본 템플릿을 복제해 시작하세요.</p><button type="button" onClick={beginCreate}>첫 템플릿 만들기</button></div>}</aside>
      <article className="template-preview">
        {mode !== "view" ? <form onSubmit={save}><p className="eyebrow">{mode === "create" ? "New template" : "Edit template"}</p><label>템플릿 제목<input autoFocus value={title} maxLength={200} onChange={(event) => setTitle(event.target.value)} /></label><label>{(mode === "edit" ? selected?.type : query.type) === "lyrics" ? "가사 구조 원문" : "쉼표로 구분한 프롬프트 토큰"}<textarea value={content} rows={14} onChange={(event) => setContent(event.target.value)} /></label><footer><button type="button" className="secondary-button" onClick={() => setMode("view")}>취소</button><button className="primary-link" type="submit">저장</button></footer></form>
