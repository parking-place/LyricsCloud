...
export function SharedLyricViewer({ actorId, initialLyric }: { actorId: string; initialLyric: SharedLyric }) {
  ...
  useEffect(() => {
    ...
    let latestAccess = initialLyric.access;
    let documentReady = false;
    let handleInstalled = false;
    const updateEditable = () => textEditor.setEditable(handleInstalled && documentReady && latestAccess.mode === "write");
    void createBrowserSharedLyricSync({ actorId, resourceId: initialLyric.id, initialAccess: initialLyric.access,
      ...
      onAccessChange(value) { if (active) { latestAccess = value; setAccess(value); updateEditable(); } },
      onReadinessChange(value) { if (active) { documentReady = value; setReady(value); updateEditable(); } },
      onPresenceChange(value) { if (active) setParticipants(value); },
      onRejectedDrafts(value) { if (active) setRejectedDrafts(value); }
    }).then((value) => { if (active) { sync.current = value; handleInstalled = true; updateEditable(); } else value.destroy(); });
    return () => {
      active = false;
  ...
  const recovery = rejectedDrafts.length ? <section className="shared-writer-recovery" aria-labelledby="shared-recovery-title">
    ...
    <p>로컬 저장 실패 또는 쓰기 권한 경계로 서버에 반영되지 않은 이 기기의 입력입니다. 먼저 원문을 복사해 별도로 보관해 주세요.</p>
