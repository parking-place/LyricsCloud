...
import { SavedResourceValidationError } from "@lyricscloud/domain";
import { RequestAuthError } from "./auth-context.js";
import { errorResponse, privateResponseHeaders } from "./http-response.js";

export function savedResponseHeaders(renewalCookie?: string): Record<string, string> {
  return { ...privateResponseHeaders, ...(renewalCookie ? { "Set-Cookie": renewalCookie } : {}) };
}
export function savedApiError(error: unknown): Response {
  ...
  if (error instanceof SavedResourceValidationError) return errorResponse("VALIDATION_FAILED", 400, undefined, error.issues);
