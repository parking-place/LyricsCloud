...
import { DisplaySettingsConflictError, DisplaySettingsValidationError } from "@lyricscloud/domain";
import { RequestAuthError } from "./auth-context.js";
import { errorResponse, privateResponseHeaders } from "./http-response.js";

export function displaySettingsResponseHeaders(renewalCookie?: string): Record<string, string> {
  return { ...privateResponseHeaders, ...(renewalCookie ? { "Set-Cookie": renewalCookie } : {}) };
}

export function displaySettingsApiError(error: unknown): Response {
  ...
  if (error instanceof DisplaySettingsValidationError) return errorResponse("VALIDATION_FAILED", 400, undefined, error.issues);
  if (error instanceof DisplaySettingsConflictError) return errorResponse("VERSION_CONFLICT", 409);
