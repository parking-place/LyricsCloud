...
import { RecentWorkValidationError } from "@lyricscloud/domain";
import { RequestAuthError } from "./auth-context.js";
import { errorResponse, privateResponseHeaders } from "./http-response.js";

export function recentWorkResponseHeaders(renewalCookie?: string): Record<string, string> {
  return { ...privateResponseHeaders, ...(renewalCookie ? { "Set-Cookie": renewalCookie } : {}) };
}

export function recentWorkApiError(error: unknown): Response {
  ...
  if (error instanceof RecentWorkValidationError) {
