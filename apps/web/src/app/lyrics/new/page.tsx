...
export default async function NewLyricPage({ searchParams }: { searchParams: Promise<{ songId?: string; returnTo?: string; template?: string }> }) {
  ...
  return <WorkspaceShell profile={user} active="songs" currentSongId={currentSongId}>
    <LyricNewScreen currentSongId={currentSongId} returnTo={safeWorkspaceReturnTo(query.returnTo)} templateId={templateId} />
  </WorkspaceShell>;
}
