...
import { parsePublicLinkAccessInput } from "../../../../../../lib/public-sharing-api.js";
import { mutationOriginAllowed } from "../../../../../../lib/song-api.js";

export const dynamic = "force-dynamic";
export const runtime = "nodejs";

export async function PATCH(request: Request, context: { params: Promise<{ lyricId: string; linkId: string }> }): Promise<Response> {
  try {
    ...
    const result = await getAuthContext().publicLyricSharing.setAccess(auth.userId, lyricId, linkId,
      parsePublicLinkAccessInput(await request.json()));
    if (!result) return errorResponse("NOT_FOUND", 404);
    return Response.json(result, { headers: { ...publicSharingResponseHeaders,
