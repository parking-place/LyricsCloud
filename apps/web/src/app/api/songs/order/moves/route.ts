import { parseSongMoveInput } from "@lyricscloud/domain";
...
export async function POST(request: Request): Promise<Response> {
  try {
    ...
    const input = parseSongMoveInput(await request.json());
    const result = await getAuthContext().songs.moveSong(auth.userId, input);
