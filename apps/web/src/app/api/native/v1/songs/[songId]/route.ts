...
export async function GET(request: Request, { params }: { params: Promise<{ songId: string }> }): Promise<Response> {
  try {
    ...
    if (!validSongId(songId)) return Response.json({ error: { code: "NOT_FOUND" } }, { status: 404, headers: nativeResponseHeaders });
    const song = await getAuthContext().songs.getSong(auth.userId, songId);
    return song ? Response.json({ song }, { headers: nativeResponseHeaders })
