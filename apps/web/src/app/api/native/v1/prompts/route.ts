...
export async function GET(request: Request): Promise<Response> {
  try {
    ...
    return Response.json(await getAuthContext().prompts.listPrompts(auth.userId,
      parsePromptListInput(new URL(request.url).searchParams)), { headers: nativeResponseHeaders });
