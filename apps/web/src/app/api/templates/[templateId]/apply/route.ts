import { isResourceId, parseApplyTemplateInput } from "@lyricscloud/domain";
...
export async function POST(request: Request, { params }: { params: Promise<{ templateId: string }> }): Promise<Response> {
  try {
    ...
    const result = await getAuthContext().templates.applyTemplate(auth.userId, templateId, parseApplyTemplateInput(await request.json()));
