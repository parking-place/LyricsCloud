...
const config: NextConfig = {
  ...
  generateBuildId: async () => process.env.NEXT_BUILD_ID ?? "lyricscloud-1.1.7a",
  ...
  async headers() {
    ...
    const privateRoutes = [
      ...
      "/search", "/recent", "/favorites", "/templates", "/trash", "/settings", "/shared/public"
    ];
    return [
      { source: "/:path*", headers: security },
      { source: "/shared/public", headers: [{ key: "X-Robots-Tag", value: "noindex, nofollow, noarchive" }] },
