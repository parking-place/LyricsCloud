...
const GUEST_SESSION_DIGEST_DOMAIN = "lyricscloud-public-guest-session-v1\0";
...
export function parseGuestSessionToken(value: unknown): string {
  if (typeof value !== "string" || !PUBLIC_SHARE_TOKEN.test(value)) throw new PublicShareTokenError();
  return value;
}

export function guestSessionTokenDigest(value: unknown): string {
  const token = parseGuestSessionToken(value);
  return createHash("sha256").update(GUEST_SESSION_DIGEST_DOMAIN).update(token).digest("hex");
}
