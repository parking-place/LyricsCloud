...
const EXPORT_SECTIONS = [
  ...
  { name: "profileAvatarPhotos", query: `select to_jsonb(q) data from (select id,encode(webp_bytes,'base64') webp_base64,content_sha256,created_at from profile_avatar_photos where owner_id=app_current_user_id() order by created_at,id) q` },
