...
describe("runtime configuration", () => {
  it("accepts a PostgreSQL URL", () => {
    ...
    expect(config).toMatchObject({ runtime: "test", appVersion: "1.1.7a", buildId: "local", appChannel: "release", appPhase: null, uiVariant: "b1" });
  });
  ...
  it("requires the sealed version and source SHA in production", () => {
    ...
    expect(readRuntimeConfig({ ...base, APP_VERSION: "1.1.7a", BUILD_ID: "a".repeat(40) }).buildId).toBe("a".repeat(40));
  ...
  it("validates explicit release channels and development phase labels", () => {
    const base = { NODE_ENV: "test", DATABASE_URL: "postgresql://user:test@db/app", APP_VERSION: "1.1.7a" };
