...
현재 소스는 `1.0.4` 프롬프트 문장 표시·복사 안내 후보이며 P1~P4 교차 인수를 완료하고 P5 최종 검증 중이다. 현재 운영 버전은 `v1.0.3`이다.
...
| 항목 | 상태 |
|---|---|
| 소스·runtime version | `1.0.4` |
| 현재 작업 | [1.0.4 Phase 5 — 최종 후보 인수](<./0.Plans/2.Patch-phase/1.0.4/5phase.md>) |
| 실행 상태 원본 | [STATUS.md](<./0.Plans/1. Dev-phase/STATUS.md>) |
| 정식 릴리스 | `v1.0.3` ([1.0.3 release notes](./docs/releases/1.0.3.md)) |
| 개발 인수 | 1.0.4 lossless 문장 표시·공통 복사 길이 안내 P1~P4, PostgreSQL·PC/mobile·Chromium/Firefox/WebKit PASS; P5 CI 진행 중 |
| 운영 제한 | 외부 암호화 backup·24시간 RPO·복원 훈련은 사용자 승인 예외로 아직 미구축 |

1.0.3은 원문 보존·capability 차단을 포함한 P1~P5와 정식 릴리스를 완료했다. 1.0.4는 저장 schema를 바꾸지 않고 문장형 프롬프트를 마침표 포함 lossless 구간으로 보여 주며, 최종 복사 payload가 1,000 Unicode code point를 넘을 때만 비차단 안내한다.
...
| 릴리스 검토자 | [1.0.3 release notes](./docs/releases/1.0.3.md), [1.0.4 후보 추적](./docs/architecture/1.0.4-FINAL-TRACEABILITY.md), [CHANGELOG](./CHANGELOG.md) |
...
1.0.5 이후 가사 copy·송폼 suffix·사전·폰트·공유·native 앱 후보는 [최신 요구 대응표](./docs/planning/latest-requirements-mapping.md)에 분리되어 있으며, 현재 1.0.4 표시·복사 안내 후보 인수와는 별도다.
