...
현재 소스는 `1.0.12` 개인 창작 흐름 통합 안정화의 Phase 5 최종 인수를 완료하고 정식 릴리스를 준비 중이다. `v1.0.10`은 정식 릴리스됐고 1.0.11은 공식 metadata 제공 경로가 없어 제품 릴리스를 만들지 않았다.
...
| 현재 작업 | [1.0.12 Phase 5 — 문서·개발 인수·후속 연결](<./0.Plans/2.Patch-phase/1.0.12/5phase.md>) |
| 실행 상태 원본 | [STATUS.md](<./0.Plans/1. Dev-phase/STATUS.md>) |
| 정식 릴리스 | 현재 `v1.0.10`; 승인된 `v1.0.12` main·tag·image·릴리스 서버 절차는 P5 통과 뒤 실행 |
| 개발 인수 | 1.0.12 P1~P4, 실제 DB 330건·Actions 전체 CI·동일 SHA 공개 수직 흐름/owner 격리/재시작 PASS |
| 운영 제한 | 외부 암호화 backup·24시간 RPO·복원 훈련은 사용자 승인 예외로 아직 미구축 |

1.0.12는 곡·라임·프롬프트 목록의 위치 계산을 결과 동등 O(n) 경로로 바꾸고, 기존 작성·복사·정렬·Suno 수동 링크를 desktop/mobile 개인 수직 흐름으로 함께 검증한다.
...
Suno 자동 메타데이터, 사전·폰트·공유·native 앱 후보는 [최신 요구 대응표](./docs/planning/latest-requirements-mapping.md)에 분리되어 있으며 현재 1.0.12 개인 흐름 안정화와는 별도다.
