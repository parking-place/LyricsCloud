...
## [Unreleased] — 1.1.8 P4 웹 안정화

- 설정 저장 응답을 기다리는 동안 새로 바꾼 표시값을 보존하고, 가사별 설정 초기화 충돌을 초기화 동작으로 다시 처리합니다.
- 필터·검색어 변경 전 목록 응답을 무시하고, 검색 키보드 이동을 화면 순서에 맞추며 곡 즐겨찾기·고정의 동시 변경을 보존합니다.
- 휴지통 처리가 성공한 뒤 목록 갱신만 실패한 경우 두 결과를 구분하고 목록을 다시 불러올 수 있습니다.
- 미저장 편집 입력의 이탈·업데이트 보호를 보완하고, 저장 실패 시 제목·원문·메모를 복구용으로 복사합니다. 일반 Suno용 복사 형식은 유지합니다.
- 공개 링크 쓰기 권한 회수 때 승인되지 않은 입력을 본문·캐시에서 분리하고, 작성자의 복구 내용은 보존합니다. 다시 쓰기를 허용해도 거부된 입력을 자동 전송하지 않습니다.
- 노트북·모바일 폭에서 편집 도구 글자가 눌리거나 잘리지 않도록 배치하고, 상태 알림이 탐색 버튼 클릭을 가리지 않게 합니다. 같은 메뉴의 재선택·PWA 자산 중복 요청과 정상 연결 표시를 정리합니다.
- 가사 제목·메모와 라임 제목의 미전송 초안을 계정별로 보관하고 재진입 때 복원·복사를 제공합니다. 한글 조합 중 저장과 복제 연타를 막고, 조합 교체 범위 안의 다른 탭 입력을 보존합니다.
- Suno 저장 응답 뒤 추가 입력, 템플릿 형식별 초안, 즐겨찾기 실패 복원과 곡 정보의 부분 저장 안내를 보정합니다. 비수동 목록 정렬에서 이동 실패와 이전 검색의 재시도를 막고 사용자 글꼴을 미리보기에도 적용합니다.
- 곡 영구 삭제의 전체 영향과 내보내기 검증을 바로잡고, 라임 잠금 순서와 동기화 투영 재시도 처리를 보완합니다. 가입 코드 만료·키 변경 검사와 신규 설치 안내를 보강합니다.
- 셸 탐색의 전체 문서 재로드와 중복 사용자 조회를 줄이고, 중첩 대화상자의 Escape 순서 및 이전 탭이 사용하는 PWA 정적 캐시 보존을 보완합니다.
- Windows 실제 기기·서명 인수와 P5·정식 릴리스는 완료되지 않았습니다.
...
P5 Actions `34864142932`·`34864188502`, main CI `34868654180`, tag CI `34871969616`과 동일 SHA 개발/운영 공개 인수를 통과했다. main/annotated tag `f8d24c6`의 정식 네 image를 exact digest로 배포하고 GitHub Release를 공개했다. 기존 DB volume·secret·allowlist·beta code와 전역 관리자 명령을 보존했다. Known limitations: 실제 OS/물리 기기·OS IME/AT/OS zoom은 새로 실행하지 않았고 `OPS-100-001` 외부 backup 예외를 유지한다.
...
- Actions `34583462005` P5 전체 verify·네 dev image 게시/서명과 동일 SHA 개발 공개 이동·재진입·재시작 순서 복원 통과

P1~P5 후보 인수를 완료했고 사용자가 승인한 정식 릴리스를 실행한다. Known limitations: 실제 물리 기기는 이번 후보에서 새로 실행하지 않았고 `OPS-100-001` 외부 backup 예외를 유지한다.
